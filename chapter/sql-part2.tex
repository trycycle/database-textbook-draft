...
第\ref{chap:sql-part1}章で解説したSQLは，1つのテーブルに対する問い合わせ（単純質問）を対象としていました．
しかし，関係データベースを用いたデータ管理や分析の現場では，問い合わせ時に参照するテーブルが1つしかないことはむしろ稀です．
データを正しく矛盾なく管理するためには，第\ref{chap:er-model}章や第\ref{chap:normalization}章で解説したような方法を用いて，データを複数のテーブルに分割して管理するからです．
そのため，関係データベースから欲しいデータを抽出するには，\strong{複数のテーブルを組み合わせた問い合わせ}を行う必要があります．

本章では，複数のテーブルを組み合わせた問い合わせを行うために，以下について解説します．
\begin{itemize}
    \item 集合演算
    \item 結合演算
    \item 副問い合わせ（サブクエリ）
...
\section{準備}
第\ref{chap:sql-part1}章と同様，以下の説明では，SQLite\footnote{\url{https://www.sqlite.org/index.html}}と解説用に作成した仮想データを用いて，SQLについて解説します．
...
ダウンロードしたファイル\graybox{sql2-example.db}は，SQLiteのデータベースファイルです．
DB Browser for SQLiteなどのSQLiteクライアントソフトを用いて，本章用の関係データベースを操作することができます．

ダウンロードしたファイルは，第\ref{chap:sql-part1}章で取り上げた小売店ショッピングサイトを管理するための関係データベースを拡張したものです．
この関係データベースには，以下のテーブルが格納されています．
\begin{itemize}
    \item テーブル「商品」：「商品ID」「商品名」「カテゴリ」「メーカー」「価格」「在庫数」を属性として持つテーブル
    \item テーブル「ユーザ」：「ユーザID」「氏名」「email」「住所」「アカウント作成年」を属性として持つテーブル
    \item テーブル「注文」：「注文ID」「ユーザID」「商品ID」「数量」「注文日」を属性として持つテーブル．属性「ユーザID」「商品ID」は，それぞれテーブル「ユーザ」の属性「ユーザID」，テーブル「商品」の属性「商品ID」に対する外部キーとなっている
    \item テーブル「関西人気商品」：関西地区で売り上げの多い上位50件の商品を抽出したテーブル．在庫数を除き，テーブル「商品」と同じ属性を持つ
    \item テーブル「関東人気商品」：関東地区で売り上げの多い上位50件の商品を抽出したテーブル．在庫数を除き，テーブル「商品」と同じ属性を持つ
\end{itemize}
参考のため，テーブル「ユーザ」とテーブル「注文」の内容を表\ref{tab:user-table}，表\ref{tab:order-table}に示します．
\begin{table}[!htbp]
    \centering
    \caption{テーブル「ユーザ」の内容．}
    \scalebox{0.9}{
    \begin{tabular}{llllr}
    \toprule
    \textbf{ユーザID} & \textbf{氏名} & \textbf{email} & \textbf{住所} & \textbf{アカウント作成年} \\ \midrule
...
\end{quotation}
のようなことを勉強したと思います．
\strong{集合演算}とは，2つの集合から別の集合を作り出す演算です．
高校数学で習った集合演算では主として数値の集合を対象としていますが，
...
そもそも，関係データベースの理論的な基盤である「関係モデル」は，数学的な集合論を土台としています．
%そのため，SQLによるテーブル操作は，根本的には集合や関係に対する数学的な演算そのものなのです．
そのため，SQLによるテーブル操作は，根本的には集合や関係に対する数学的な演算に基づいています（ただし，SQLには実用上の理由で要素の重複を許す拡張があります）．
...
\end{itemize}
この条件を満たしていない場合，レコード集合の集合演算を行うSQL文を実行するとエラーが出力されます．
...
SQLで和集合を求めるには\graybox{UNION}演算子を用います．
レコード集合$A$とレコード集合$B$の和集合を求めるには，\graybox{A UNION B}のようなSQL文を書きます．
$A$や$B$はレコード集合であるので，実際にSQL文で\graybox{UNION}を使う際には，$A$や$B$の箇所に\strong{SELECT文が入ります}．

コード\ref{code:sql-union}は，テーブル「関東人気商品」とテーブル「関西人気商品」の和集合を求めるSQL文です．
...
\begin{figure}[!htbp]
    ...
    \caption{テーブル「関東人気商品」とテーブル「関西人気商品」の和集合を求めるSQL文}
    \label{code:sql-union}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「関東人気商品」とテーブル「関西人気商品」同士の和集合演算（SQL）の結果}
    \scalebox{0.99}{
    \begin{tabular}{lllll}
    \toprule
...
なお，（後述する積集合，差集合も同様ですが）和集合演算子\graybox{UNION}の演算対象となるSELECT文には，\graybox{WHERE}句などを含んでいても問題ありません．
コード\ref{code:sql-union2}は，テーブル「関東人気商品」とテーブル「関西人気商品」のレコードのうち，属性「価格」の値が\graybox{1000}円以上のレコードに限定して和集合を求めるSQL文です．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{属性「価格」の値が\graybox{1000}円以上のレコードに限定して，テーブル「関東人気商品」とテーブル「関西人気商品」の和集合を求めるSQL文}
    \label{code:sql-union2}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{属性「価格」の値が\graybox{1000}円以上のレコードに限定して，テーブル「関東人気商品」とテーブル「関西人気商品」同士の和集合を求めるSQL文の実行結果}
    \scalebox{0.99}{
    \begin{tabular}{lllll}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} \\
...
レコード集合$A$とレコード集合$B$の積集合を求めるには，\graybox{A INTERSECT B}のようなSQL文を書きます．
コード\ref{code:sql-intersect}は，テーブル「関東人気商品」とテーブル「関西人気商品」の積集合を求めるSQL文です．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「関東人気商品」とテーブル「関西人気商品」の積集合を求めるSQL文}
    \label{code:sql-intersect}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「関東人気商品」とテーブル「関西人気商品」の積集合を求めるSQL文の実行結果}
    \scalebox{0.99}{
    \begin{tabular}{lllll}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} \\
...
\strong{差集合演算}は，あるレコード集合から別のレコード集合に含まれるレコードを除いたレコードの集合（差集合，set difference）を求めるものです．
高校数学の範囲で例えると，
\begin{quotation}
...
SQLで差集合を求めるには\graybox{EXCEPT}演算子を用います．
レコード集合$A$とレコード集合$B$の差集合を求めるには，\graybox{A EXCEPT B}のようなSQL文を書きます．
コード\ref{code:sql-except}は，テーブル「関東人気商品」からテーブル「関西人気商品」を引いた差集合を求めるSQL文です．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「関東人気商品」からテーブル「関西人気商品」を引いた差集合を求めるSQL文}
    \label{code:sql-except}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「関東人気商品」からテーブル「関西人気商品」を引いた差集合を求めるSQL文の実行結果}
    \scalebox{0.99}{
    \begin{tabular}{lllll}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} \\
...
%和集合演算や積集合演算とは異なり，「交換法則」は\strong{成立しません}．
和集合演算（$A \cup B = B \cup A$）や積集合演算（$A \cap B = B \cap A$）では交換法則が成立するのに対して，差集合演算では「交換法則」は\strong{成立しません}．
つまり，$A \setminus B$と$B \setminus A$はイコールになるとは限りません．
差集合演算は演算の項の順序を入れ替えると結果が変わります．

例えば，コード\ref{code:sql-except2}はコード\ref{code:sql-except}のSQL文とは演算の順序を入れ替え，テーブル「関西人気商品」からテーブル「関東人気商品」を引いた差集合を求めるSQL文です．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「関西人気商品」からテーブル「関東人気商品」を引いた差集合を求めるSQL文}
    \label{code:sql-except2}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「関西人気商品」からテーブル「関東人気商品」を引いた差集合を求めるSQL文の実行結果}
    \scalebox{0.99}{
    \begin{tabular}{lllll}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} \\
...
第\ref{chap:sql-part1}章で説明したように，関係データモデルは集合論に基づいているため，要素（タプル）の重複は許されません．
一方，SQLは問い合わせ結果の中ではレコードの重複を許します．
...
関係データベースではデータの正しさを担保するために，データを複数のテーブルに分けて管理します．
そのため，\strong{結合操作を使いこなすこと}が多様かつ膨大なデータの管理・検索・分析を行う上での\strong{鍵}となります．

以下，直積，内部結合，外部結合の3つの結合操作について説明します．


\subsection{直積（CROSS JOIN）}
集合論において，集合AとBの\strong{直積}\footnote{直積はデカルト積と呼ばれることもあります．}とは，集合Aと集合Bのそれぞれの要素のすべての組み合わせ（の集合）を意味します．
...
関係データベースにおける\strong{直積（交差結合; cross join）} は，2つのテーブル$R_1$と$R_2$が与えられたとき，テーブル$R_1$のレコードと$R_2$のレコードのすべての組み合わせを求める操作となります．
...
\end{figure}
上記SQL文は，\strong{CROSS JOIN}をカンマ（,）で置き換えてコード\ref{code:sql-cross-join2}のように書くこともできます．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
例えば，表\ref{tab:subject}と\ref{tab:teacher}のようなテーブル「科目」とテーブル「教員」の2つがあるとします．
表が示すとおり，テーブル「科目」には3レコード，テーブル「教員」には4レコード格納されています．
\begin{table}[!htbp]
    \centering
    \caption{科目テーブル}
    \scalebox{0.99}{
...
2つのテーブルの直積を求めると，表\ref{tab:cross-join-result}のように，合計$3 \times 4=12$レコードが得られます．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「科目」とテーブル「教員」の直積を求めるSQL文}
    \label{code:sql-cross-join3}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「科目」とテーブル「教員」の直積を求めるSQL文の実行結果}
    \scalebox{0.99}{
    \begin{tabular}{llll}
    \toprule
...
直積を求めるSQL文では，\graybox{WHERE}句を使って抽出結果を選択（絞り込み）することができます．
例えば，テーブル「商品」とテーブル「注文」の直積を求めた後，属性「商品ID」の値が一致しているレコードだけを抽出してみましょう．
...
\begin{figure}[!htbp]
    ...
    WHERE
        -- 「商品.商品ID」とすることで，
        -- テーブル「商品」の属性「商品ID」の値を参照することを明示．
        -- 「注文.商品ID」とすることで，テーブル「注文」の属性「商品ID」の値を参照することを明示．
        商品.商品ID = 注文.商品ID;

    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」とテーブル「注文」の直積を求めた後，属性「商品ID」の値が一致しているレコードだけを抽出するSQL文}
    \label{code:sql-cross-join4}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」とテーブル「注文」の直積を求めた後，属性「商品ID」の値が一致しているレコードだけを抽出するSQL文の実行結果}
    \scalebox{0.64}{
    \begin{tabular}{llllrrlllrl}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} & \textbf{注文ID} & \textbf{ユーザID} & \textbf{商品ID} & \textbf{数量} & \textbf{注文日} \\
    \midrule
P001 & 国産カレーパン & パン & A製パン & 270 & 206 & O0102 & U091 & P001 & 8 & 2025-08-11 \\
P001 & 国産カレーパン & パン & A製パン & 270 & 206 & O0113 & U015 & P001 & 4 & 2025-09-15 \\
P001 & 国産カレーパン & パン & A製パン & 270 & 206 & O0726 & U011 & P001 & 2 & 2025-03-29 \\
P001 & 国産カレーパン & パン & A製パン & 270 & 206 & O0742 & U049 & P001 & 10 & 2022-03-10 \\
P001 & 国産カレーパン & パン & A製パン & 270 & 206 & O1165 & U033 & P001 & 5 & 2021-07-10 \\
& & $\vdots$ & & & $\vdots$ & & \\
P500 & 辛口めんつゆ & 調味料 & E食品 & 590 & 19 & O1390 & U052 & P500 & 7 & 2019-02-15 \\
    \bottomrule
    \end{tabular}
    }
...
例えば，前節「直積」で扱ったテーブル「商品」とテーブル「注文」について，属性「商品ID」で内部結合してみましょう．
コード\ref{code:inner-join}は，テーブル「商品」とテーブル「注文」を内部結合するSQL文です．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」とテーブル「注文」を内部結合するSQL文}
    \label{code:inner-join}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」とテーブル「注文」を内部結合するSQL文の実行結果}
    \scalebox{0.64}{
    \begin{tabular}{llllrrlllrl}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} & \textbf{注文ID} & \textbf{ユーザID} & \textbf{商品ID} & \textbf{数量} & \textbf{注文日} \\
...
前節の直積を利用したSQL文（コード\ref{code:sql-cross-join4}）と同じ結果が出力されましたが，内部的な処理手順は異なります．
直積のSQL文はいったん直積ですべての組み合わせを求めた後，属性「商品ID」の値が一致するものを調べて結果を表示します．
これに対し，内部結合を用いた上記SQL文では，\graybox{ON}句で指定した条件を満たす組み合わせのみを抽出し，その後それをすべて選択（表示）するという順で処理を行っています．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{USING句を使ってテーブル「商品」とテーブル「注文」を内部結合するSQL文}
    \label{code:sql-inner-join-using}
\end{figure}


（後述する外部結合も含めて）\graybox{JOIN}句と\graybox{WHERE}句を組み合わせる際には，\graybox{JOIN}の後に\graybox{WHERE}句が適用されるのを意識することが重要となります．
例えば，先の内部結合の例（コード\ref{code:inner-join}）において「数量が1の注文に限定したい」としましょう．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{属性「数量」の値を絞り込んでテーブル「商品」とテーブル「注文」を内部結合するSQL文}
    \label{code:sql-inner-join-where}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{属性「数量」の値を絞り込んでテーブル「商品」とテーブル「注文」を内部結合するSQL文の実行結果}
    \scalebox{0.6}{
    \begin{tabular}{llllrrllrl}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} & \textbf{注文ID} & \textbf{ユーザID} & \textbf{数量} & \textbf{注文日} \\
    \midrule
P182 & じっくり煮込んだトマトジュース & 飲料 & Fファーム & 200 & 452 & O0001 & U006 & 1 & 2024-02-10 \\
P432 & 糖質オフビーフカレー & レトルト & Mフーズ & 600 & 178 & O0006 & U083 & 1 & 2025-07-16 \\
P399 & 天然麻婆豆腐の素 & レトルト & C食品 & 190 & 78 & O0045 & U090 & 1 & 2022-03-04 \\
P330 & プレミアムごまドレッシング & 調味料 & Q醤油 & 490 & 376 & O0084 & U052 & 1 & 2021-02-05 \\
P269 & たっぷりパスタソース & レトルト & Mフーズ & 470 & 336 & O0093 & U055 & 1 & 2024-11-21 \\
& & $\vdots$ & & & $\vdots$ & & \\
P320 & 職人仕込みフルーツグミ & 菓子 & K製菓 & 400 & 410 & O1991 & U036 & 1 & 2024-11-16 \\
    \bottomrule
    \end{tabular}
    }
...
\subsection{外部結合（OUTER JOIN）}\label{sec:outer-join}
内部結合は，片方のテーブル$R_1$にあるレコード$r$に対して，もう片方のテーブル$R_2$のレコードに結合条件を満たす列データがあるときに限ってレコード同士を結合し，その結果を表示するものです．
これは，結合条件を満たすレコードがもう片方のテーブル$R_2$にない場合は$r$は\strong{無視される}ことを意味します．

本章で例題として用いているテーブル「注文」には，テーブル「商品」にある商品に関する注文が記録されています．
実は，テーブル「商品」に登録された商品の中には，テーブル「注文」に記録されたことのない商品，つまり注文されたことがない商品も存在します．
そのため，テーブル「商品」とテーブル「注文」を内部結合すると，「注文されたことのない商品」に関するレコードが結合結果に残りません．

内部結合とは異なり，結合条件を満たさないレコードがあった時でもそのことを示す情報を付与したレコードを残しつつ，テーブル同士を結合する演算が\strong{外部結合（outer join）} です．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」とテーブル「注文」を（左）外部結合するSQL文}
    \label{code:sql-outer-join}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」とテーブル「注文」を（左）外部結合するSQL文の実行結果}
    \scalebox{0.6}{
    \begin{tabular}{llllrrllrl}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} & \textbf{注文ID} & \textbf{ユーザID}  & \textbf{数量} & \textbf{注文日} \\
    \midrule
P001 & 国産カレーパン & パン & A製パン & 270 & 206 & O0102 & U091 & 8 & 2025-08-11 \\
P001 & 国産カレーパン & パン & A製パン & 270 & 206 & O0113 & U015 & 4 & 2025-09-15 \\
P001 & 国産カレーパン & パン & A製パン & 270 & 206 & O0726 & U011 & 2 & 2025-03-29 \\
P001 & 国産カレーパン & パン & A製パン & 270 & 206 & O0742 & U049 & 10 & 2022-03-10 \\
P001 & 国産カレーパン & パン & A製パン & 270 & 206 & O1165 & U033 & 5 & 2021-07-10 \\
& & $\vdots$ & & & $\vdots$ & & \\
P142 & こだわり和風ドレッシング & 調味料 & Fファーム & 880 & 187 & NULL & NULL & NULL & NULL \\
& & $\vdots$ & & & $\vdots$ & & \\
P365 & 無添加のど飴 & 菓子 & I製菓 & 370 & 284 & NULL & NULL & NULL & NULL \\
& & $\vdots$ & & & $\vdots$ & & \\
P500 & 辛口めんつゆ & 調味料 & E食品 & 590 & 19 & O1390 & U052 & 7 & 2019-02-15 \\
    \bottomrule
    \end{tabular}
    }
    \label{tab:outer-join-result}
\end{table}
内部結合の例とは異なり，「注文されたことのない商品」に関するレコードが表示され，属性「注文ID」「ユーザID」「数量」「注文日」の値が\graybox{NULL}になっていることが分かります．

内部結合は両方のテーブルにデータがあるものだけを残すため，例えば「注文されなかった商品」に関するレコードはSQL実行結果に現れません．
一方，「注文されなかった商品の特徴」などを分析するために，注文履歴がない「商品」のレコードも見たい場合がよくあります．
このように「結合するデータがなかった場合でも，メインのデータ（レコード）も出力結果に残しておきたい」というときに大活躍するのが外部結合です．
外部結合は，結合条件を満たすレコードがなかったとしてもそれを把握したい時などに活用できます．

なお，外部結合には\strong{左外部結合（left outer join）} と\strong{右外部結合（right outer join）}，および\strong{完全外部結合（full outer join）} の3種類が存在します\footnote{SQLiteでは，バージョンによっては右外部結合および完全外部結合はサポートされていないので注意しましょう．}．
...
SQLでは\graybox{NULL}と空文字を厳格に区別しているため，扱いには注意しましょう．
\end{notebox}


\section{副問い合わせ}\label{sec:subquery}
関係データベースへの問い合わせが複雑になってくると，SQLによる問い合わせ結果を使ってさらにSQL文を書きたいケースも出てきます．
このようなケースで役に立つのが\strong{副問い合わせ（サブクエリ; subquery）} です．
...
テーブル「注文」とテーブル「商品」を用いて，以下のフローを実現するSQL文を書いてみましょう．
\begin{enumerate}
...
このSQL文の実行結果は表\ref{tab:sales-by-product-result}のようになります．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
もし上記\ref{tab:sales-by-product-result}のテーブルが「売上」という名で存在していれば，コード\ref{code:sales-top10-sql}のようなSQL文を書くことで目的を達成できます．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    -- これは仮のSQL文．実際にはテーブル「売上」は存在しない
    SELECT * FROM 売上 ORDER BY 売り上げ額 DESC LIMIT 10;

    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「売上」が存在する場合の上位10件の商品情報を出力するSQL文}
    \label{code:sales-top10-sql}
\end{figure}

上記のステップ1と2は，ステップ1のSQL文を「副問い合わせ」することで実現できます．
具体的には，コード\ref{code:sales-top10-sql}のSQL文の\graybox{FROM}句にある「売上」の箇所を，ステップ1に関するSQL文（コード\ref{code:sql-sales-by-product}）をカッコ\graybox{( )}で囲んだもので置き換えます．
このSQL文の実行結果は，表\ref{tab:sales-top10-result}のようになります．

長いSQL文を見ると難しく感じるかもしれませんが，焦る必要はありません．
副問い合わせのコツは，「まずは必要なデータがまとまった小さな仮想テーブル（今回はテーブル「売上」）を作り，それに対して改めてSELECTを行う」という2段階のステップで考えることです．
...
そのため，副問い合わせ箇所に\graybox{AS}修飾句で名前をつけたり，\graybox{WITH}句を使って副問い合わせ箇所を外出しするなど，少しでもSQL文を分かりやすくする工夫が重要です\footnote{\graybox{WITH}句は副問い合わせの仲間ではありますが，厳密には共通テーブル式（CTE: Common Table Expression）と呼びます．}．

コード\ref{code:subquery-sales-top10-with-with}のSQL文は，先のSQL文を\graybox{AS}修飾句や\graybox{WITH}句を使ってわかりやすく書き換えたものです．
コード\ref{code:subquery-sales-top10}のSQL文よりも多少は読みやすくなったのではないでしょうか．
...
\section{EXISTS述語とIN述語}
\subsection{EXISTS述語}
先の例では\graybox{FROM}句内で副問い合わせ指定を行っていましたが，\graybox{WHERE}句においても副問い合わせを行うことができます．
例えば，これまでの例で用いた3つのテーブル「関西人気商品」「商品」「注文」を用いて，
\begin{quotation}
テーブル「注文」から，テーブル「関西人気商品」には\strong{入っていない}商品に関するもので，かつ注文数量が5以上のレコードを抽出する
\end{quotation}
ようなSQL文を考えてみましょう．
ちなみに，
\begin{quotation}
テーブル「注文」から，テーブル「関西人気商品」には\strong{入っている}商品に関するもので，かつ注文数量が5以上のレコードを抽出する
\end{quotation}
ようなSQL文は簡単です．
コード\ref{code:subquery-where-in}のように，単純にテーブル「関西人気商品」とテーブル「注文」を結合すればよいからです．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「注文」から，テーブル「関西人気商品」に入っている商品に関するもので，かつ注文数が5以上のレコードを抽出するSQL文}
    \label{code:subquery-where-in}
\end{figure}

一方，関西人気商品リストに「入っていない」商品の注文の場合，以下のような処理の流れになります．
\begin{enumerate}
\item テーブル「注文」を参照して，テーブル「商品」のレコードと結合する
\item テーブル「商品」内の各レコードの「商品ID」の値が，テーブル「関西人気商品」内に存在するかを判定する
\item 「関西人気商品」内に商品がなかったレコードで，注文数が5以上のレコードのみを抽出する
\end{enumerate}
上記ステップ2では「別のテーブルを活用した対象レコードの存在判定」を行っています．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「注文」において，テーブル「関西人気商品」には入っていない商品に関するもので，かつ注文数が5以上のレコードを抽出するSQL文}
    \label{code:subquery-where-not-exists}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「注文」において，テーブル「関西人気商品」には入っていない商品に関するもので，かつ注文数が5以上のレコードを抽出するSQL文の実行結果}
    \scalebox{0.72}{
...
なお，\graybox{リスト}の箇所は\graybox{(値, 値, 値)}のようにリストの要素をカッコ内に並べてベタ書きで指定してもよいですし，副問い合わせで指定してもよいです．

例えば，\graybox{NOT EXISTS}述語を用いたコード\ref{code:subquery-where-not-exists}のSQL文は，\graybox{IN}述語を使ってコード\ref{code:subquery-where-not-in}のように書き換えることができます．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
\graybox{IN}述語は，副問い合わせが返す「すべての結果のリスト」の中に注目している値が含まれるかを判定します．
一方，\graybox{EXISTS}述語は，条件に合致するレコードが「少なくとも1つ存在するか」を判定し，合致するレコードが1つ見つかった時点で判定を終えます．
...
第\ref{chap:sql-part1}章と本章で学んだSQLは，ISOなどによって国際的に標準化されています．
しかし，実際のRDBMS（SQLite，MySQL，PostgreSQLなど）においては，標準規格に完全に準拠しているわけではなく，独自の拡張や関数（いわゆる「方言」）が存在します．
SQLの標準規格（SQL:1999など）やその変遷についてより深く理解したい場合は，DateとDarwenによる解説書\cite{Date1997}やMeltonとSimonの著書\cite{Melton1993}などの標準的なリファレンスが非常に参考になるでしょう．
\end{tipbox}
...
本クイズでは，第\ref{chap:sql-part1}章のクイズと同様，独立行政法人統計センターが公開している教育用標準データセット（SSDSE）の基本素材SSDSE-E\footnote{教育用標準データセット（SSDSE）: \url{https://www.nstac.go.jp/use/literacy/ssdse/\#SSDSE-E}}から，著者が抜粋・加工したデータを用います．
加工済みのデータのファイル（\graybox{SSDSE.db}）はこちらのURL（\url{https://github.com/trycycle/database-textbook/raw/refs/heads/main/dataset/SSDSE.db}）よりダウンロードできます．
クイズで利用するテーブルは以下の3つとなります．
\begin{itemize}
\item テーブル``population''
\item テーブル``university\_student\_population\_top10''
\item テーブル``activity''
\end{itemize}

テーブル``population''は，第\ref{chap:sql-part1}章のクイズで用いたものです．
テーブルの内容は表\ref{tab:population-table}を参照してください．

表\ref{tab:university_student_population_top10}のとおり，テーブル``university\_student\_population\_top10''には，大学学生数が上位10位内であった都道府県の「地域コード」「都道府県」「総人口」が格納されています．

\begin{table}[!htbp]
    \centering
    \caption{テーブル``university\_student\_population\_top10''の内容}
    \scalebox{0.99}{
    \begin{tabular}{rrr}
    \toprule
    \textbf{地域コード} &
...
表\ref{tab:activity}のように，テーブル``activity''には，「学習・自己啓発・訓練」「スポーツ」「趣味・娯楽」「ボランティア」「旅行・行楽」について過去1年以内に活動したことのある人の割合（％）が，都道府県別にまとめられています．
\begin{table}[!htbp]
    \centering
    \caption{テーブル``activity''の内容}
    \scalebox{0.75}{
...
テーブル``population''にあるレコードのうち，列「大学学生数」の値が\graybox{7万}以上かつ「小学校児童数」の値が\graybox{30万}以上の都道府県名を表示するSQL文を書きなさい．
...
\subsection*{Q3. 直積}
テーブル``population''とテーブル``activity''の直積を求めるSQL文を書きなさい．

\subsection*{Q4. 内部結合}
テーブル``population''とテーブル``activity''を列「地域コード」の値が等しいという条件で内部結合を行うSQL文を書きなさい．
ただし，結果表示時には重複する列（すなわち「地域コード」「都道府県」）は除くこと．

\subsection*{Q5. 直積再び}
上記Q4と同様の内容を表示するSQL文を\graybox{JOIN}を用いずに書きなさい．

\subsection*{Q6. 外部結合}
テーブル``university\_student\_population\_top10''に対して列「地域コード」の値が等しいという条件でテーブル``activity''を左外部結合しなさい．

\subsection*{Q7. 副問い合わせ（1/2）}
テーブル``population''において調査年度が\graybox{2021}時点の小学校児童数上位20件のレコードを求め，それらとテーブル``activity''の内容を結合した内容を表示するSQL文を書きなさい．

\subsection*{Q8. 副問い合わせ（2/2）}
テーブル``activity''において「ボランティア」の値が上位10件以内に入る都道府県を求めるSQL文を考えなさい．
その上で，テーブル``population''を用いて，「ボランティア」の値が上位10件以内に入る各都道府県の調査期間（2020および2021）における平均総人口を求めるSQL文を書きなさい．
