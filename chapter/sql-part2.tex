...
\begin{itemize}
    ...
    \item テーブル「ユーザ」：「ユーザID」「氏名」「email」「住所」「アカウント作成日」を属性として持つテーブル
    \item テーブル「注文」：「注文ID」「ユーザID」「商品ID」「数量」「注文日」を属性として持つテーブル．属性「ユーザID」「商品ID」は，それぞれテーブル「ユーザ」の属性「ユーザID」，テーブル「商品」の属性「商品ID」に対する外部キーとなっている
    \item テーブル「関西人気商品」：関西地区で売り上げの多い上位50件の商品を抽出したテーブル．テーブル「商品」と同じ属性を持つ
    \item テーブル「関東人気商品」：関東地区で売り上げの多い上位50件の商品を抽出したテーブル．テーブル「商品」と同じ属性を持つ
\end{itemize}
...
例えば，先の内部結合の例（\ref{code:inner-join}）において「数量が1の注文に限定したい」としましょう．
...
例えば，内部結合の例で用いた3つのテーブル「関西人気商品」「商品」「注文」を用いて，
\begin{quotation}
テーブル「注文」から，テーブル「関西人気商品」には\strong{入っていない}商品に関するもので，かつ売り上げ数量が5以上のレコードを抽出する
\end{quotation}
ようなSQL文を考えてみましょう．
ちなみに，
\begin{quotation}
テーブル「注文」から，テーブル「関西人気商品」には\strong{入っている}商品に関するもので，かつ売り上げ数量が5以上のレコードを抽出する
\end{quotation}
ようなSQL文は簡単です．
