...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{RDBMSではないデータベース管理システム（NoSQL）}:
    \begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{交通系ICカードのサービス}（例：SuicaやICOCAなど）：利用者の属性情報，乗降履歴，電子マネーの残高情報などをデータベースで管理している．
\end{enumerate}

\subsubsection*{Q4. CSV/TSVファイル}
\textbf{CSVファイル}（Comma-Separated Values）は，各行のデータ項目をカンマ（,）で区切ったテキストファイルである．\textbf{TSVファイル}（Tab-Separated Values）は，区切り文字にタブ文字を使う点がCSVファイルと異なる．どちらも表形式のデータを保存・共有するために広く使われている形式である．

CSV/TSVファイルをDBMSの代わりにデータ管理に用いた場合の主な欠点：
\begin{enumerate}
    \item \textbf{大規模データの高速処理が困難}：特定のレコードを探すには原則としてファイルを先頭から全件読み込む必要があり，データ量が増えると処理が著しく遅くなる．
...
\begin{enumerate}
    \item $(\text{`B21001'},\; \text{`山田 太郎'},\; \text{`工学部'},\; -1,\; \text{`愛知県'})$：属性「年齢」の値 $-1$ が $Dom(\text{年齢})$ に含まれていない（年齢が負）のでドメイン制約違反．
    \item $(\text{`21001'},\; \text{`田中 花子'},\; \text{`文学部'},\; 20,\; \text{`東京都'})$：属性「学籍番号」の値が `B' で始まっておらず $Dom(\text{学籍番号})$ に含まれていないためドメイン制約違反．
    \item $(\text{`B22001'},\; \text{`鈴木 一郎'},\; \text{`スポーツ学部'},\; 21,\; \text{`名古屋'})$：「スポーツ学部」は $Dom(\text{学部})$ に含まれておらず，「名古屋」は市名であり $Dom(\text{出身都道府県})$ にも含まれていないため，2つのドメイン制約違反が生じている．
\end{enumerate}

\subsubsection*{Q6. 超キーと候補キー}
関係スキーマ $\boldsymbol{R}(A, B, C, D)$ において主キーが $\{A, B\}$ であるということは，$\{A, B\}$ は候補キーでもあることを意味する．各選択肢を検討する：
\begin{enumerate}
    ...
    \item $\{A, C\}$：$A$ と $C$ の組み合わせでタプルを一意に識別できるかどうかは問題文の情報からは判断できない．そのため，他の選択肢から確実に「超キーだが候補キーでない」と判断できるものだけを選ぶ．
...
\begin{enumerate}
    \item 制約条件から導かれる関数従属性を以下にすべて列挙する：
        \begin{itemize}
            ...
            \item 「同姓同名の従業員がいてもよい」より，従業員名 $\to$ 従業員番号 が成り立たない（従業員名は一意識別子にならない）．
            \item 「1つの部門には複数名の従業員が所属する」より，部門コード $\to$ 従業員番号 が成り立たない（部門コードだけでは従業員を特定できない）．
        \end{itemize}
...
まず，単一属性を候補として検討する：
\begin{itemize}
    \item \textbf{店舗コード}：S01が2行，S02が2行存在する → 一意でない．
    \item \textbf{店舗名}：「星ヶ丘」が2行，「八事」が2行存在する → 一意でない．
    \item \textbf{会員番号}：1が3行，2が2行存在する → 一意でない．
    \item \textbf{会員名}：「川澄 桜」がS02とS03の2行に存在する → 一意でない．
    \item \textbf{会員種別}：同一値が複数存在する → 一意でない．
\end{itemize}

次に，2属性の組み合わせを候補として検討する：
\begin{itemize}
    \item $\{\text{店舗コード}, \text{会員番号}\}$：表中の5行はすべて一意であり，店舗コードのみ・会員番号のみでは一意識別できない（最小性を満たす）． → \textbf{候補キー}
    \item $\{\text{店舗名}, \text{会員番号}\}$：表中の5行はすべて一意であり，店舗名のみ・会員番号のみでは一意識別できない（最小性を満たす）． → \textbf{候補キー}
\end{itemize}

以上より，候補キーは $\{\text{店舗コード}, \text{会員番号}\}$ と $\{\text{店舗名}, \text{会員番号}\}$ の2つが存在する．
このうち，\textbf{主キーとして適切なのは $\{\text{店舗コード}, \text{会員番号}\}$} である．
店舗コードは人工的に割り当てられた識別子であり変更されにくいのに対して，店舗名は店舗の移転・改名によって変更される可能性があり，主キーには不向きである．
...
\end{align*}
1つ目の制約は，テーブル「商品」の属性「商品カテゴリID」に格納される値が，テーブル「商品カテゴリ」の主キーとして必ず存在することを表す制約である．2つ目から4つ目の制約 はテーブル「購買」の各外部キー属性（顧客ID，店舗ID，商品ID）の値が，それぞれの参照先テーブルの主キーとして必ず存在することを表す制約である．
...
\begin{minted}[gobble=0,
    ...
    SELECT 都道府県, 調査年度, 大学学生数 FROM population LIMIT 30;
...
「300万人」は数値 \graybox{3000000} として記述し，「県で終わる」には \graybox{LIKE} 演算子で対応する．
\begin{minted}[gobble=0,
    bgcolor=background,
    fontsize=\small,
    linenos=false,
...
\begin{minted}[gobble=0,
    ...
    SELECT DISTINCT 都道府県 FROM population
    WHERE 総人口 < 3000000
    AND 都道府県 LIKE '%県'
    AND 中学校生徒数 <= 高等学校生徒数;
...
\begin{minted}[gobble=0,
    ...
    SELECT * FROM population WHERE 調査年度 = 2021
    ORDER BY 大学学生数 DESC;

\end{minted}

\subsubsection*{Q6. ソート (2/2)}
SQLiteでは割合を計算する際，整数同士の除算では小数点以下が切り捨てられるため，\graybox{CAST} で実数（REAL型）に変換してから除算する．
\begin{minted}[gobble=0,
    bgcolor=background,
    fontsize=\small,
    linenos=false,
...
テーブル「履修登録」の主キーは \{学生ID, 科目ID\} であるため，どちらか一方の属性，または両方の属性の値が\graybox{NULL}のレコードは登録できない．各トラブルの分類は以下の通り：
\begin{enumerate}
    \item \textbf{挿入時異状}．新設科目「データサイエンス入門」はまだ誰も履修登録していないため，主キーの一部である「学生ID」の値が\graybox{NULL}となり，キー制約違反でレコードを挿入できない．本来，科目の開設と学生の履修登録は独立したできごとであるにもかかわらず，テーブルの設計上，履修者なしには科目情報を登録できないという不都合が生じている．
...
ここで，ある営業担当者（例：担当者「鈴木」）の今年度の営業成績が修正された場合を考える．担当者「豊田」が複数の（ユーザ，商品）ペアを担当していれば，テーブル「営業記録」には担当者「豊田」が登場する行が複数存在する．よって，営業成績の修正に際しては，「豊田」が担当しているすべての行の「営業成績」の値を書き換えなければならない．

しかし，修正の際にいずれかの行を見落とした場合，テーブル「営業記録」内で同一の担当者「豊田」に対して異なる営業成績の値が混在することになる（例：ある行では営業成績「A」，別の行では「B」）．これが更新時異状であり，どちらの値が正しいのか判断できない矛盾した状態を引き起こす．

\subsubsection*{Q3. 削除時異状}
テーブル「営業記録2」では，各行にユーザの連絡先（連絡先）と取扱商品のメーカー（メーカー）の情報が含まれている．これらの情報は，特定の（ユーザ，商品）ペアのレコードと同じ行に混在して管理されている．

ここで，あるユーザ（例：ユーザ「tanabee」）が特定の商品（例：商品「オフィスデスク」）の購入相談をキャンセルしたため，対応するレコードをテーブルから削除する場面を考える．もし「tanabee，オフィスデスク」のレコードが，テーブル「営業記録2」における「オフィスデスク」についての唯一の記録であった場合，そのレコードを削除すると「オフィスデスク」のメーカー情報とユーザ「tanabee」の連絡先情報もデータベースから失われてしまう．

本来「ユーザの問い合わせキャンセル」という操作にすぎないにもかかわらず，商品・メーカー・ユーザの連絡先という別のデータが意図せず消去されるのが削除時異状である．

\subsubsection*{Q4. 挿入時異状}
テーブル「営業記録3」の主キーは \{ユーザ, 商品\} であるため，何らかのユーザが商品に関心を示して初めてレコードを挿入できる．

ここで，家具販売店「八高家具」が新しいメーカー（例：メーカー「北欧家具社」）の新商品（例：商品「ダイニングテーブル」）を取り扱い品目に追加した場面を考える．このとき，その商品に興味を持った顧客がまだ1人もいなければ，「ユーザ」の値が\graybox{NULL}となりキー制約違反となるため，「北欧家具社」と「ダイニングテーブル」の対応関係をテーブル「営業記録3」に登録することができない．
...
第\ref{chap:er-model}章の図\ref{fig:erd-to-schema}のER図を変換ルールに従い，以下の6つの関係スキーマに変換する（参照整合性制約は省略）．
\begin{itemize}
...
\begin{table}[!htbp]
    ...
    山畑 & 白日           & King Gnu         & CEREMONY              & J-Pop & 5 \\
    山畑 & Bohemian Rhapsody & Queen         & A Night at the Opera & Rock  & 4 \\
    田辺 & 白日           & King Gnu         & CEREMONY              & J-Pop & 3 \\
...
そのため，図のように$FD_4$で分解した後，左辺が共通の$FD_2$と$FD_3$（楽曲→\{収録アルバム, ジャンル\}）をまとめて分解する．

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{figure/quiz-answer/ch7-q1-3.pdf}
    \caption{7章Q1-3に対応するFDダイアグラムの分解過程}
    \label{fig:ch7-q1-3}
\end{figure}
...
\end{equation*}
候補キーは\{ユーザ, 商品\}．$FD_2$，$FD_3$，$FD_4$の順で分解する．
分解過程は以下の図の通り．

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{figure/quiz-answer/ch7-q2.pdf}
    \caption{7章Q2に対応するFDダイアグラムの分解過程}
    \label{fig:ch7-q2}
\end{figure}

最終的に得られる下記4つのBCNFスキーマは，本文の例（$FD_4$，$FD_2$，$FD_3$の順）で得られた$R_6 \sim R_9$と完全に一致する：
\begin{align}
    &\boldsymbol{R_a}(\underline{\text{ユーザ, 商品}},\; \text{営業担当}) \\
    &\boldsymbol{R_b}(\underline{\text{ユーザ}},\; \text{連絡先}) \\
    &\boldsymbol{R_c}(\underline{\text{商品}},\; \text{メーカー}) \\
    &\boldsymbol{R_d}(\underline{\text{営業担当}},\; \text{営業成績})
\end{align}
この例では，$FD_2$，$FD_3$，$FD_4$の順で分解しても，本文で示した分解例と同じBCNFスキーマが得られる．
ただし，一般には分解に用いる関数従属性の順序によって，最終的に得られるスキーマが異なる場合がある．
...
\begin{figure}[!htbp]
    ...
    \caption{7章Q3に対応するFDダイアグラムの分解過程}
    \label{fig:ch7-q3}
\end{figure}
...
%     \caption{7章Q4に対応するFDダイアグラムの分解過程}
%     \label{fig:ch7-q4}
% \end{figure}
