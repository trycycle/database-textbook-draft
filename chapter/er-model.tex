...
第\ref{chap:anomaly}章では，データの挿入，削除，更新時に発生するデータベースの不都合である異状について説明しました．
データベースの異状を含め，データベースを常に正しく矛盾がない状態に保つためには，第\ref{chap:relational-data-model}章で説明した関係データモデルの制約やテーブルの構造を適切に設計することが重要になってきます．

ところが，データベースを正しく設計することは想像以上に難しいことです．
これまでの章で説明に用いてきたテーブルは高々2，3個でした．
しかし，データ管理の現場では関係データベース内のテーブルの数が数十個以上になることはめずらしくありません．
また，データベース設計においては唯一の正解があるわけではなく，現実世界の複雑なルールやデータが用いられる文脈を紐解き，それらをデータベース管理システムが扱える形に落とし込む必要があります．
...
%そこで本章および次章では，関係データベースの設計手法である\strong{実体関連モデルの変換による関係スキーマの設計方法}および\strong{正規化}について解説します．
そこで本章では，\strong{実体関連モデルの変換による関係スキーマの設計方法}について解説します．


\section{データモデリングの流れ}
第\ref{chap:concept-of-database}章で説明したように，データモデリングとは対象とする課題に応じて扱うべき情報を取捨選択し，データの構造，制約条件，操作方法を設計する行為です．
...
\begin{itemize}
    ...
    \item データ従属性に基づく正規化：ある程度具体的なデータ項目（テーブルの属性）が出揃っている状況で，データの重複や異状が発生しないテーブル構造を導出したい場合
\end{itemize}
...
設計は知識よりも経験（考え方の体得）が効いてきます．
データモデリングのような抽象的な思考を体得するには，とにかく「やっては間違えてを繰り返す」しかありません．
\end{notebox}



\section{実体関連モデルとは}\label{sec:what-is-er-model}
\strong{実体関連モデル（entity-relationship model; ERモデル）} は，Peter Chen氏により提唱された概念モデリング用のデータモデルです\cite{Chen1976}．
実体関連モデルでは，実世界のデータのすべてを「実体」と「関連」の2種類で分類・記述します．
実体関連モデルを図として表現したものを\strong{実体関連図（entity-relationship diagram; ER図）} と呼びます．

実体関連モデルおよび実体関連図には様々な記法\footnote{本書で説明するPeter Chen記法以外には，カラス足記法，IDEF1X記法などが存在します．}や拡張\cite{Batini1991}が存在しますが，本書ではPeter Chenの記法に従い説明を行います．

\subsection{実体}
\strong{実体（entity）}とは，データ対象をモデル化しようとしたときに，独立した存在として一意に識別可能な物体や事象です．
例えば，ショッピングサイトで取り扱う商品や購買記録をデータベースで管理することを考えてみましょう．
そこに登場する個々のユーザや商品，販売者，商品レビュー，カートなどはすべて実体と見なせます．
...
つまり，実体は実体集合内で必ず主キーで一意に識別できる必要があります．
...
\item 属性は楕円
\end{itemize}
で表現し，実体型とその属性は線で結びます．
また，主キーとなる属性には\strong{下線を引きます}．

図\ref{fig:product_entity}は，あるショッピングサイトにおける実体型「商品」を実体関連図として表現したものです．
仮にこのような実体型「商品」を設計した場合，このショッピングサイトで取り扱うすべての商品は必ず「商品ID」「商品名」「価格」「発売日」を属性として持つ（そして，これら以外の属性は持たない）ことがルールとして定められたことになります．
\begin{figure}[!htbp]
    \centering
...
例えば，関連「お気に入り」について考えると，お気に入り（として登録する）という行為は行為主体であるユーザと行為対象である商品がそろって初めて定義できます．
...
\end{itemize}
で表現します．
また，関連型とその属性は線で結び，関連型はつながりのある実体型と線で結びます．
...
実体関連モデルにおける関連は，数学的には「実体の主キーの組み合わせ」で表現されます．
...
このベン図において，点は各集合の要素（つまり実体や関連）を表しています．
...
そのため，図の赤バツ印の箇所のように，ユーザ\graybox{川澄}が商品\graybox{はーいお茶}を2回目のお気に入り登録することができません．
...
例えば，山畑滝子さんという学生には，川澄桜子さんという先輩と田辺通さんという後輩がいたとしましょう．
この状況を先に定義した実体関連モデルで考えてみると，図\ref{fig:er_model_without_role}のように，「山畑滝子」という唯一無二の実体が実体集合「先輩学生」と実体集合「後輩学生」のそれぞれの要素に現れてしまいます（\graybox{S2}と\graybox{J1}）．
...
先輩も後輩も同じ「学生」なのだから，実体型「学生」だけを使い，適切な関連型を定義することで先輩後輩関係を表現できないのでしょうか．
...
実体関連モデルを提唱したPeter Chen氏は自身が発表した論文\cite{Chen1983}において，以下のような対応付けの経験則を提案しています．
\begin{itemize}
\item 普通名詞: 実体型
\item 固有名詞: 実体
\item 他動詞: 関連型
...
\item 副詞: 関連の属性
\end{itemize}
以上の経験則は\strong{言語分析に基づく実体関連モデル抽出法}の基礎として知られています．
言語分析に基づく実体関連モデル抽出法は，シナリオから実体関連モデルを作成する際の一つの方法論として有用です．
一般的には，以下のステップを経て実体関連モデルを抽出します．
\begin{enumerate}
\item シナリオ文章を構成する文を単語に分割し，品詞を特定する
\item 文の主語，述語，目的語の関係を抽出する
\item 出現頻度の高い名詞や，特定の動詞の主語・目的語となる名詞を「実体型」の候補とする
\item 実体同士を結びつける動詞を特定し，「関連型」候補とする
\item 名詞を修飾する形容詞や自動詞（例：○○は○○を持つ）から，実体型の「属性」を抽出する
\item 動詞を修飾する副詞から，関連型の「属性」を抽出する
\end{enumerate}

実際のシナリオでは上記の経験則がうまく当てはまらないこともありますが，一定の目安にはなります．
実体関連モデルを習い始めたばかりの人は参考にしてみてください．
\end{notebox}


\section{実体関連モデルにおける一貫性制約}
関係データモデルでは，対象となるデータが満たすべき制約としてドメイン制約やキー制約といった一貫性制約を定義します．
...
図の実体型「商品」に注目すると，どの実体「ユーザ」とも関連を持たない実体「商品」\graybox{（P3）}（商品名：タケノコの山）が存在していることが分かります．
...
実体関連図において，実体型$E_1$と$E_2$が関連型$R$で結ばれているとします．
このとき，モデリングの対象となる問題設定によっては，$E_1$のすべての要素が，$R$を通じて必ず$E_2$のいずれかの要素とつながっていてほしい場合があります．
このように，$E_1$のすべての要素が関連に参加するケースを，$E_1$の$R$への参加は\strong{全体的}であると呼びます．
実体関連図上でこの「全体的」な参加を表現する際は，実体型$E_1$と関連型$R$を結ぶ線を\strong{太線}にすることで明示します．
例えば，先のショッピングサイトの例で考えてみましょう．
実体型「商品」の関連型「お気に入り」への参加が「全体的」である場合は，図\ref{fig:participation_constraint}のような実体関連図になります．
\begin{figure}[!htbp]
    \centering
...
\end{figure}
図\ref{fig:participation_constraint}のように定義することで，どの実体「商品」も，関連「お気に入り」を通じて必ずいずれかの実体「ユーザ」とつながっていなければならないという要件を表現できます．

逆に，$E_1$のすべての要素が，$R$を通じて$E_2$の要素とつながっている必要がない場合，$E_1$の$R$への参加は\strong{部分的}であると呼びます．
実体関連図上でこの「部分的」な参加を表現する際は，実体型$E_1$と関連型$R$を結ぶ線を\strong{細線}にします．
例えば，先のショッピングサイト（図\ref{fig:er_model_elements2}）のケースで考えてみましょう．
誰からも「お気に入り」に登録されていない商品が存在しても構わない場合，図\ref{fig:participation_constraint}における実体型「商品」の関連型「お気に入り」への参加は「部分的」となります．
このように，ある実体型が関連型へ参加する際，それが全体的か部分的かを定める制約のことを\strong{参加制約（participation constraint）}と呼びます．
...
\subsection{多重度制約}
ショッピングサイトの例に戻りましょう．
お気に入り登録に加えて，各商品がどのメーカーによって製造されたかの情報についても管理する実体関連モデルを設計したとします．
図\ref{fig:er_model_elements3}は，その実体関連モデルに基づき実体集合「ユーザ」「商品」「メーカー」および関連集合「お気に入り」「製造」の要素を図示したものです．
\begin{figure}[!htbp]
    \centering
...
\end{figure}
この図が示す内容をそのまま読み取ると，以下のことが分かります．
\begin{enumerate}
...
このように，ある実体が関連を通じていくつの実体と対応づけられるかを示す数を\strong{多重度（cardinality）}\footnote{Cardinalityは多重度と呼ばれることもありますが，語源からすると「濃度」が正しい訳語とする説もあります．}と呼びます．
また，この多重度に関する制約を\strong{多重度制約（cardinality constraint）}と呼びます．

関連の多重度は，大きく分けて「1対1」「1対多」「多対多」に分類されます．
例えば，上記図においては
\begin{itemize}
...
関連型「お気に入り」から実体型「ユーザ」への線は直線に，関連型「製造」から実体型「メーカー」への線は矢印線になっています．
\begin{figure}[!htbp]
    \centering
...
このようなケースに対応するため，サイト運営者は図\ref{fig:cardinality_constraint}の実体関連図を改善して，
\begin{itemize}
\item 実体型「家族」を追加
\item 実体「ユーザ」と実体「家族」が同居関係にあることを示すために，関連型「同居」を追加
\end{itemize}
することを検討しているとしましょう．
この際，実体「家族」にはユーザ名は割り振られず，家族の誰かに商品を送るときは，その家族のユーザがサイトにログインをして手続きをすることを想定してください．

このようなケースでは，\strong{弱実体}という特殊な概念を用いて実体関連図を設計することがあります．
自身がもつ属性だけでは主キーを構成できない特殊な実体集合を\strong{弱実体集合（weak entity set）}と呼びます．
弱実体を特定する上で，それと関連している実体のことを\strong{所有実体（owner entity）}，弱実体と所有実体をつなげる関連を\strong{弱関連（weak relationship）}と呼びます．
また，弱実体を一意に特定するために，所有実体の主キーとセットにして使われる弱実体の属性を\strong{部分キー（partial key）}と呼びます．
弱実体集合内の弱実体は，それ単独では一意に特定できません．
所有実体と部分キーの組み合わせによって初めて，一意に特定できるようになります．
...
図\ref{fig:weak_entity}の実体関連図のようにすると，ある実体「ユーザ」を特定して初めて，関連「同居」で結ばれた実体「家族」を属性「氏名」の値によって特定できるようになります．
言い換えると，実体「ユーザ」の情報がなければ，「氏名」だけで特定の弱実体「家族」を見つけ出すことはできません．
これは，弱実体「家族」は自身のもつ属性だけでは主キーを構成できないことを意味しています．
...
結論からいえば，弱実体を用いるべきかどうかは理論的に正解があるわけではなく，対象をデータベース上でどう定義し，どのように扱いたいかという設計方針によって決まります．
...
このようなケースでは，実体型「図書」のキーを実体型「書籍」の属性「ISBN番号」と「連番」の組みあわせ（連番が部分キー）で表現すれば，「書籍」を所有実体，「図書」を弱実体として設計することができます．
一方，実体「図書」1冊1冊に図書ID（図書館専用のバーコード番号のようなもの）という一意に識別可能な値を割り振れば，理論上は「図書」を通常の実体として扱えます．
...
\section{汎化・継承}\label{sec:inheritance}
...
このとき，実体関連モデルで実体型「学生」「教員」を定義するには，どうしたらよいでしょうか．
...
このように，実体型$E_1$が持つすべての属性を持つ，新たな実体型$E_2$を定義するとき，$E_2$は$E_1$を\strong{継承（inherit）}したといいます（「$E_1$は$E_2$を\strong{汎化（generalize）}した実体型である」と称することもあります）．
...
\end{figure}
図\ref{fig:3-aryrelationship1}の左図は，学生がどこの教室でどの教員の指導をうけるかを表現した実体関連図，右図は実体関連図の各実体集合と関連集合の要素を示した図です．
図\ref{fig:3-aryrelationship1}の左図では，関連型「研究室配属」は実体型「学生」「部屋」「教員」の3項関連となっています．
また，関連型「研究室配属」において
\begin{itemize}
...
\end{itemize}
となっています．
これは，学生には必ず指導教員が1名，部屋が1つ割り当てられる，ということを意味します．
...
\end{figure}
一方，図\ref{fig:3-ary-relationship2}は，図\ref{fig:3-aryrelationship1}と似ていますが，実体関連図で表現されている内容が微妙に異なります．
図\ref{fig:3-ary-relationship2}の実体関連図では，関連型「研究室配属」において
\begin{itemize}
\item 「学生」と「教員」は多対多の関連
\item 「学生」と「部屋」は多対1の関連
\end{itemize}
となっており，その解釈には注意が必要です．
n項関連においては，ある実体集合$E$に向かう矢印は，「関連する他のすべての実体集合から実体を1つずつ選んだとき，その組みあわせに対して$E$の実体が1つに決まる」を意味すると定義されています．
この定義から，図\ref{fig:3-ary-relationship2}においては，\strong{学生と教員の組みあわせ}に対応して部屋が1つに決まる，ということを意味します．
図中の3項関係を，2項関連のように
\begin{itemize}
...
\end{itemize}
と解釈してはいけません．
この実体関連図に対応する要素に関する図（図\ref{fig:3-ary-relationship2}の右図）を見ると，学生Bには桜山先生と山畑先生の2名の教員が指導教員として割り当てられていますが，部屋割は学生と教員の組みあわせに対して行われるため，学生Bには8501号室と8601号室の2部屋が割り当てられています（異なる教員を割り当てれば，同じ学生に2部屋割り当てることができる）．


\section{実体関連モデルから関係スキーマへの変換}\label{sec:erd-to-relational-model}
...
図\ref{fig:erd-entity}に示した実体型「ユーザ」は，以下のような関係スキーマに変換されます．
\begin{figure}[!htbp]
  \centering
...
\end{figure}
\begin{equation}
  \text{\textbf{ユーザ}}(\underline{\text{ユーザ名}}, \text{氏名}, \text{住所}, \text{email})
\end{equation}
...
図\ref{fig:erd-relation1}の場合，実体型「ユーザ」の主キーは「ユーザ名」，実体型「商品」の主キーは「商品ID」であるため，関連型「お気に入り」の主キーは「ユーザ名」と「商品ID」のペアとなります\footnote{このように属性の組みあわせによって構成された主キーは複合主キーと呼ばれます．複合主キーは理論上正しいのですが，実務では管理を容易にするためにシステムが自動的に付与する人工的な主キー「代理キー（surrogate key）」を持たせることがあります．代理キーの例としては，連番や重複しない文字列（UUID）などが挙げられます．}．
結果的に，関連型「お気に入り」は以下のような関係スキーマに変換されます．
\begin{equation}
  \text{\textbf{お気に入り}}(\underline{\text{ユーザ名}, \text{商品ID}}, \text{登録日})
\end{equation}
...
例えば，先の例の関係「お気に入り」の主キーを構成する属性「ユーザ名」と「商品ID」は，関係「ユーザ」および「商品」に対する外部キーになります．
つまり関係「お気に入り」の主キーである属性「ユーザ名」と「商品ID」の値は，必ず関係「ユーザ」の属性「ユーザ名」の値，関係「商品」の属性「商品ID」の値として存在している必要があります．
よって，上記関連型を関係スキーマに変換したときには，
\begin{align}
  &\text{\textbf{ユーザ}}(\underline{\text{ユーザ名}}, \text{氏名}, \text{住所}, \text{email}) \label{eq:user}\\
  &\text{\textbf{商品}}(\underline{\text{商品ID}}, \text{商品名}, \text{価格}, \text{発売日}) \label{eq:product}\\
  &\text{\textbf{お気に入り}}(\underline{\text{ユーザ名}, \text{商品ID}}, \text{登録日}) \label{eq:favorite}\\
%  \text{お気に入り.ユーザ名} \subseteq \text{ユーザ.ユーザ名} \label{eq:favorite-fk1}\\
  &\pi_{ユーザ名}(\textbf{お気に入り}) \subseteq \pi_{ユーザ名}(\textbf{ユーザ}) \label{eq:favorite-fk1}\\
%  \text{お気に入り.商品ID} \subseteq \text{商品.商品ID} \label{eq:favorite-fk2}
  &\pi_{商品ID}(\textbf{お気に入り}) \subseteq \pi_{商品ID}(\textbf{商品}) \label{eq:favorite-fk2}
\end{align}
が参照整合性制約を含めた関係スキーマとなります．


\subsubsection{多重度が「1対多」の場合}
\begin{figure}[!htbp]
  \centering
...
図\ref{fig:erd-relation2}では，実体型「メーカー」と「商品」は関連型「製造」を介して1対多の関連でつながっています．
すなわち，実体集合「商品」の実体要素が決まれば，それに対応する実体集合「メーカー」の実体要素が必ず1つ特定されることになります．

このような「1対多」の関連型の場合，矢印のついていない側の実体の主キーの値が決まれば，矢印のついている側の実体が一意に特定されると同時に関連の要素も一意に特定されます．
そのため，関係スキーマに変換する場合は，\strong{関連型につながる，矢印のついていない側の実体型の主キー}を関連型の主キーとします．

図\ref{fig:erd-relation2}を関係スキーマに変換すると，以下のようになります．
関係「製造」において「企業名」が属性になりますが，主キーにはならない（含まれない）ことに注意しましょう．
\begin{align}
  &\text{\textbf{商品}}(\underline{\text{商品ID}}, \text{商品名}, \text{価格}, \text{発売日}) \\
  &\text{\textbf{メーカー}}(\underline{\text{企業名}}, \text{TEL}, \text{email}) \\
  &\text{\textbf{製造}}(\underline{\text{商品ID}}, \text{企業名}) \\
  %\text{製造.商品ID} \subseteq \text{商品.商品ID}\\
  &\pi_{商品ID}(\textbf{製造}) \subseteq \pi_{商品ID}(\textbf{商品}) \label{eq:manufacture-fk1}\\
  %\text{製造.企業名} \subseteq \text{メーカー.企業名}
  &\pi_{企業名}(\textbf{製造}) \subseteq \pi_{企業名}(\textbf{メーカー}) \label{eq:manufacture-fk2}
\end{align}
...
関係「ロゴ」において，属性「企業名」を主キーとして設定した場合，属性「画像ID」が属性になるが主キーにならないことに注意しましょう．
\begin{align}
  &\text{\textbf{メーカー}}(\underline{\text{企業名}}, \text{TEL}, \text{email}) \\
  &\text{\textbf{画像}}(\underline{\text{画像ID}}, \text{ファイル名}, \text{サイズ}) \\
  &\text{\textbf{ロゴ}}(\underline{\text{企業名}}, \text{画像ID}) \\
  %\text{ロゴ.企業名} \subseteq \text{メーカー.企業名}\\
  &\pi_{企業名}(\textbf{ロゴ}) \subseteq \pi_{企業名}(\textbf{メーカー}) \label{eq:logo-fk1}\\
  %\text{ロゴ.画像ID} \subseteq \text{画像.画像ID}
  &\pi_{画像ID}(\textbf{ロゴ}) \subseteq \pi_{画像ID}(\textbf{画像}) \label{eq:logo-fk2}
\end{align}


\subsection{変換パターン3：汎化・継承}
実体型$E_1$を継承した実体型$E_2$を関係スキーマに変換するには，ほぼ実体型と同じ変換方法を用います．
すなわち
\begin{itemize}
\item 継承先の実体型$E_2$の名前を関係スキーマ$E_2$の関係名
\item 継承元の実体型$E_1$のキーを関係スキーマ$E_2$のキー
\item 継承先の実体型$E_2$の属性を関係スキーマ$E_2$の属性
\end{itemize}
に対応させればよいです．
継承先の実体型（$E_2$）の主キーの設定が特殊なので注意しましょう．
...
\begin{equation}
  \text{\textbf{プレミアムユーザ}}(\underline{\text{ユーザ名}}, \text{登録日})
\end{equation}

なお，継承先の実体型の主キーは継承元の実体の主キーを借りているため，参照整合性制約が生じます．
図\ref{fig:erd-inheritance}の実体型「プレミアムユーザ」であれば，属性「ユーザ名」は実体型「ユーザ」の属性「ユーザ名」に対する外部キーになります．
そのため，以下のような参照整合性制約を関係スキーマに加える必要があります．
\begin{equation}
  \pi_{ユーザ名}(\textbf{プレミアムユーザ}) \subseteq \pi_{ユーザ名}(\textbf{ユーザ})
\end{equation}
...
\begin{figure}[!htbp]
  ...
  \caption{弱実体型「家族」と弱関連型「同居」}
  \label{fig:weak-entity-relation}
\end{figure}
図\ref{fig:weak-entity-relation}のような弱実体・弱関連の関係スキーマへの変換はややトリッキーです．
弱実体は自らの属性だけでは一意に特定できないため，弱関連型を通じてつながる（所有）実体型の主キーと自らの属性（部分キー）をセットにして主キーとする必要があります．
...
\begin{equation}
  \text{\textbf{家族}}(\underline{\text{ユーザ名}, \text{氏名}}, \text{続柄})
\end{equation}

なお，弱実体型の主キーを構成する要素のうち，部分キーでない方は別の実体型の主キーを借りているため，参照整合性制約が生じます．
先の例の弱実体型「家族」であれば，属性「ユーザ名」が外部キーに相当します．
そのため，以下のような参照整合性制約を関係スキーマに加える必要があります．
\begin{equation}
  \pi_{ユーザ名}(\textbf{家族}) \subseteq \pi_{ユーザ名}(\textbf{ユーザ})
\end{equation}
...
\begin{align}
  &\text{\textbf{同居}}(\underline{\text{ユーザ名}, \text{氏名}}) \\
  %\text{同居.ユーザ名} \subseteq \text{ユーザ.ユーザ名} \\
  &\pi_{ユーザ名}(\textbf{同居}) \subseteq \pi_{ユーザ名}(\textbf{ユーザ}) \label{eq:cohabitation-fk1}\\
  %\text{同居.氏名} \subseteq \text{家族.氏名} \\
  %&\pi_{氏名}(\textbf{同居}) \subseteq \pi_{氏名}(\textbf{家族}) \label{eq:cohabitation-fk2}
  &\pi_{ユーザ名, 氏名}(\textbf{同居}) \subseteq \pi_{ユーザ名, 氏名}(\textbf{家族}) \label{eq:cohabitation-fk2}
\end{align}
...
レコードの追加，修正，削除には，第\ref{chap:sql-part1}章で紹介した関係データベースの問い合わせ言語であるSQLを用います．
...
データ型は第\ref{chap:relational-data-model}章で説明したドメイン制約に対応するものです．
...
このような複雑な制約を\graybox{CREATE TABLE}文で指定するには，あらかじめ\graybox{CREATE DOMAIN}文でドメインを定義しておき，定義したドメインをテーブル定義のデータ型に指定する方法があります\footnote{PostgreSQLなど一部のDBMSではCREATE DOMAINが使えます．本書で扱っているSQLiteではCREATE DOMAINが使えないため，CHECK制約などで代替することになります．}．
これについては本書のレベルを超える内容になるため，ここでの説明は割愛します．

\graybox{CREATE TABLE}文における列名の\graybox{[制約]}の箇所には，その列に対する様々な制約を指定することができます．
代表的な制約は以下の通りです：
\begin{itemize}
...
主キーが複数の列の組みあわせで構成される場合など，複雑な主キーや外部キーを指定する場合は，テーブル全体の制約として指定する方法が適しています．
...
\begin{figure}[!htbp]
    ...
    CREATE TABLE お気に入り (
        ...
        -- テーブル「お気に入り」の「ユーザ名」は
        -- テーブル「ユーザ」の「ユーザ名」に対する外部キーとなる
        FOREIGN KEY (ユーザ名) REFERENCES ユーザ(ユーザ名),
        -- テーブル「お気に入り」の「商品ID」は
        -- テーブル「商品」の「商品ID」に対する外部キーとなる
...
\graybox{INSERT}文の構文はコード\ref{code:insert-into-structure}のような形式を取ります．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
コード\ref{code:insert-into-example}は，先ほど取り上げたテーブル「商品」に3件のレコードを追加するためのSQL文の例です．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
\graybox{DELETE}文の構文はコード\ref{code:delete-structure}のような形式を取ります．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
コード\ref{code:delete-example}は，先ほど取り上げたテーブル「商品」から特定のレコードを削除するためのSQL文の例です．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から列「価格」の値が\graybox{100}円以下のレコードを削除するSQL文の例}
...
\graybox{UPDATE}文の構文はコード\ref{code:update-structure}のような形式を取ります．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
コード\ref{code:update-example}は，先ほど取り上げたテーブル「商品」の中で列「商品ID」の値が\graybox{P002}のレコードの価格を1割引きしたものに修正するSQL文の例です．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」で列「商品ID」の値が\graybox{P002}のレコードの価格を1割引きした値に修正するSQL文の例}
...
以下，架空の音楽ストリーミングサービスOrange Musicの実体関連モデルを作成します．
Orange Musicの構成要素について，以下の条件を満たす実体関連図を作成しなさい．
\begin{itemize}
  \item 「ユーザ」は「ユーザID」「氏名」「性別」「誕生日」「電話番号」を持つ．
  \item 「楽曲」は「楽曲ID」「楽曲名」「ジャンル」「長さ」を持つ．
  \item 「アーティスト」は「アーティストID」「アーティスト名」を持ち，作成した「楽曲」を「公開」する．「公開」には「公開日」が記録される．
  \item 「ユーザ」は「プレイリスト」（属性：「プレイリストID」「プレイリスト名」）を「作成」し，その際「作成日」が記録される．
  \item 「プレイリスト」には「楽曲」を「追加」でき，「追加された日」が記録される．
\end{itemize}

\subsubsection{Q2. 多重度と参加制約}
Q1で作成した実体関連図に実体型「レーベル」（属性：「レーベル名」「住所」）および関連型「いいね」を追加し，以下の設定を読み解いて各関連の「多重度」および「参加制約（全体的か部分的か）」を反映させなさい．

\begin{itemize}
  \item 「アーティスト」は「レーベル」に「所属」することができる．ただし，所属できる「レーベル」は1つだけである．「レーベル」には何組かの「アーティスト」が「所属」する．
  \item 「楽曲」公開時には必ず「アーティスト」が1名登録され，「アーティスト」はいくつかの「楽曲」を「公開」する．
  \item ユーザは「プレイリスト」を作成することができる．複数の「ユーザ」が協力し合って「プレイリスト」を作成することはない．「プレイリスト」はOrange Musicのスタッフが作成することがある（「ユーザ」に紐付かない「プレイリスト」が存在する）．
  \item 「ユーザ」は気に入った「楽曲」に「いいね」をすることができる．「ユーザ」は何曲でも「いいね」でき，「いいねされた日（liked\_at）」が記録される．
\end{itemize}

\subsubsection{Q3. 汎化・継承}
%\subsubsection{Q3. 弱実体と汎化・継承}
Orange Musicは追加サービスとして「プレミアムユーザ」サービスを開始した．
「ユーザ」は追加料金を支払うことで「プレミアムユーザ」になることができる．
「プレミアムユーザ」は最大100曲まで「楽曲」を「ダウンロード」でき，その際「ダウンロード日」が記録される．
汎化・継承の概念を用いて，この状況をQ2の実体関連図に追加しなさい．

% \begin{enumerate}
%   \item これまでに作成した実体関連図において「弱実体」および「弱関連」になり得るものを特定し，図に反映しなさい（弱実体の部分キーには点線を付与すること）．
%   \item Orange Musicは追加サービスとして「プレミアムユーザ」サービスを開始した．「ユーザ」は追加料金を支払うことで「プレミアムユーザ」になることができる．「プレミアムユーザ」は最大100曲まで「楽曲」を「ダウンロード」でき，その際「ダウンロード日」が記録される．この汎化・継承の構造を図に追加しなさい．
% \end{enumerate}

\subsubsection{Q4. n項関連の設計}
Orange Musicではさらなる機能追加として，アーティストがコンサート等のイベントで演奏した曲目（いわゆるセットリスト）を閲覧できるサービスを開始することにした．以下の要件を満たすよう，実体関連図を拡張しなさい．

\begin{framed}
「アーティスト」がどの「楽曲」をどの「イベント」で「演奏」したのかを閲覧できる．
...
\subsubsection{Q5. 実体関連モデルから関係スキーマへの変換}
