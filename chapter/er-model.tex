...
データベースの異状を含め，データベースを常に正しく矛盾がない状態に保つためには，\ref{chap:relational-data-model}章で説明した関係データモデルの制約やテーブルの構造を適切に設計することが重要になってきます．
...
データ型は\ref{chap:relational-data-model}章で説明したドメイン制約に対応するものです．
...
コード\ref{code:update-example}は，先ほど取り上げたテーブル「商品」の中で列「商品ID」の値が\graybox{P002}のレコードを価格を1割引きしたものに修正するSQL文の例です．
\begin{figure}[h]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
\begin{itemize}
  ...
  \item 「ユーザ」は別の「ユーザ」を「フォロー」し，その際「フォロー日」が記録される．
\end{itemize}

\subsubsection{Q2. 多重度と参加制約}
Q1で作成した実体関連図に「レーベル」（属性：「レーベル名」「住所」）を追加し，以下の設定を読み解いて各関連の「多重度」および「参加制約（全体的か部分的か）」を反映させなさい．

\begin{itemize}
  \item 「アーティスト」はいずれか1つの「レーベル」に「所属」し，「レーベル」には何組かの「アーティスト」が「所属」する．
  \item 「楽曲」公開時には必ず「アーティスト」が1名登録され，「アーティスト」はいくつかの「楽曲」を「公開」する．
  \item ある「プレイリスト」を作成した「ユーザ」は必ず一人である．
  \item 「ユーザ」は気に入った「楽曲」に「いいね」をすることができる．「ユーザ」は何曲でも「いいね」でき，「いいねされた日（liked\_at）」が記録される．
\end{itemize}

\subsubsection{Q3. 弱実体と汎化・継承}
Q2の図をさらに精緻化するため，以下の作業を行いなさい．
...
\subsubsection{Q6. 実体関連モデルから関係スキーマへの変換}
