...
\ref{chap:relational-data-model}章まで定義の話が続いてきたので，本章からは関係データベースを扱うための実践的な話題に移ろう．
...
\begin{table}[h]
    ...
    P001 & 激辛 チョコカレーパン & パン & B製菓 & 380 & 206 \\
    P002 & 国産 クロワッサン & パン & Dフーズ & 460 & 443 \\
    P003 & 無添加 ハンバーグ & レトルト & C飲料 & 370 & 149 \\
    P004 & 激辛 ビーフカレー & レトルト & A食品 & 410 & 166 \\
    P005 & 特製 味噌 & 調味料 & Eファーム & 250 & 18 \\
    &        &     $\vdots$    &       &        &       \\
    P500 & メガ盛り オリーブオイル & 調味料 & C飲料 & 240 & 19 \\ \bottomrule
    \end{tabular}
    }
...
\begin{table}[h]
    ...
    特製 味噌  & Eファーム & 250 \\
    &   $\vdots$      &      \\
    メガ盛り オリーブオイル & C飲料 & 240 \\ \bottomrule
    \end{tabular}
    \label{tb:sql-select}
\end{table}
...
\begin{table}[h]
    ...
    P003 & 無添加 ハンバーグ & レトルト & C飲料 & 370 & 149 \\
    P004 & 激辛 ビーフカレー & レトルト & A食品 & 410 & 166 \\
    P005 & 特製 味噌 & 調味料 & Eファーム & 250 & 18 \\ \bottomrule
    \end{tabular}
    }
...
\begin{table}[h]
    ...
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
    P011 & 国産 コーンスープ & レトルト & C飲料 & 590 & 299 \\
    P015 & 国産 ドレッシング & 調味料 & Eファーム & 500 & 57 \\
    P016 & 濃厚 オリーブオイル & 調味料 & Eファーム & 500 & 145 \\
    P030 & メガ盛り プロテインバー & 菓子 & Dフーズ & 500 & 117 \\
    P034 & 激辛 チョコビーフカレー & レトルト & Eファーム & 760 & 207 \\
    &        &       $\vdots$    &     &        &       \\
    P494 & ゼロカロリー クロワッサン & パン & B製菓 & 570 & 28 \\ \bottomrule
    \end{tabular}
    }
    \label{tab:sql-where}
\end{table}
...
コード\ref{code:sql-where2}は，「商品」テーブルから「メーカー」が「Eファーム」のレコードを抽出するSQL文の例です．
このSQL文を実行すると，表\ref{tab:sql-where2}のように，「メーカー」列の値が「Eファーム」のレコードのみが抽出されます．
\begin{figure}[h]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{「商品」テーブルからメーカーが「Eファーム」のレコードを抽出するSQL文}
    \label{code:sql-where2}
\end{figure}
\begin{table}[h]
    \centering
    \caption{「商品」テーブルからメーカーが「Eファーム」のレコードを抽出した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}llllll@{}}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
    P005 & 特製 味噌 & 調味料 & Eファーム & 250 & 18 \\
    P010 & 激辛 コーラ & 飲料 & Eファーム & 190 & 26 \\
    P014 & メガ盛り 炭酸水 & 飲料 & Eファーム & 120 & 37 \\
    P015 & 国産 ドレッシング & 調味料 & Eファーム & 500 & 57 \\
    P016 & 濃厚 オリーブオイル & 調味料 & Eファーム & 500 & 145 \\
    &               & $\vdots$     &   &        &       \\
    P499 & メガ盛り チョコクッキー & 菓子 & Eファーム & 370 & 203 \\ \bottomrule
    \end{tabular}
    }
...
\begin{table}[h]
    ...
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
    P001 & 激辛 チョコカレーパン & パン & B製菓 & 380 & 206 \\
    P007 & 無添加 チョコチップパン & パン & B製菓 & 380 & 95 \\
    P009 & 国産 チョコチップパン & パン & B製菓 & 160 & 295 \\
    P013 & 有機 チョコクッキー & 菓子 & B製菓 & 450 & 198 \\
    P022 & 完熟 チョコチーズリゾット & レトルト & C飲料 & 390 & 135 \\
    &               & $\vdots$     &   &        &       \\
    P499 & メガ盛り チョコクッキー & 菓子 & Eファーム & 370 & 203 \\ \bottomrule
    \end{tabular}
    }
...
コード\ref{code:sql-where4}は，「商品」テーブルからメーカーが「A食品」でかつ価格が500以上のレコードを抽出するSQL文の例です．
このSQLを実行すると，表\ref{tab:sql-where4}のように，「メーカー」列の値が「A食品」でかつ「価格」列の値が500以上のレコードが抽出され，その「商品名」「価格」「在庫数」の値が表示されます．
\begin{figure}[h]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{「商品」テーブルから「メーカー」が「A食品」でかつ「価格」が500以上のレコードを抽出するSQL文（表示する列は「商品名」「価格」「在庫数」限定）}
    \label{code:sql-where4}
\end{figure}
\begin{table}[h]
    \centering
    \caption{「商品」テーブルから「メーカー」が「A食品」でかつ「価格」が500以上のレコードを抽出し，「商品名」「価格」「在庫数」の値を表示した結果}
    \scalebox{1.00}{
    \begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{商品名} &  \textbf{価格} & \textbf{在庫数} \\ \midrule
濃厚 チョコミックスナッツ &  520 & 355 \\
特製 コーンスープ & 730 & 477 \\
国産 シチュー & 700 & 297 \\
有機 ビーフカレー & 760 & 192 \\
濃厚 チョコハンバーグ & 610 & 298 \\
 &               $\vdots$          &       \\
 激辛 チョコチキンカレー & 610 & 353 \\ \bottomrule
    \end{tabular}
    }
...
\begin{table}[h]
    ...
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
P074 & 贅沢 メロンパン & パン & B製菓 & 380 & 2 \\
P084 & 完熟 チョコシチュー & レトルト & Dフーズ & 460 & 2 \\
P012 & 贅沢 野菜ジュース & 飲料 & B製菓 & 180 & 3 \\
P125 & ゼロカロリー チョコハンバーグ & レトルト & Dフーズ & 360 & 3 \\
P157 & 完熟 コーンスープ & レトルト & Eファーム & 640 & 3 \\
 \bottomrule
    \end{tabular}
    }
...
\begin{table}[h]
    ...
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
P089 & 無添加 ポテトチップス & 菓子 & Eファーム & 370 & 500 \\
P179 & 国産 プロテインバー & 菓子 & A食品 & 420 & 500 \\
P370 & 国産 ビーフカレー & レトルト & C飲料 & 560 & 498 \\
P364 & 国産 ビーフカレー & レトルト & Eファーム & 670 & 496 \\
P422 & 濃厚 プロテインバー & 菓子 & Eファーム & 170 & 496 \\
 \bottomrule
    \end{tabular}
    }
...
例えば，「商品」テーブルから「メーカー」がB製菓のレコードだけに限定して，商品の「在庫数」の合計値を計算することを考えましょう．
...
\begin{figure}[h]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{「商品」テーブルから「メーカー」がB製菓のレコードに限定して「在庫数」の合計値を計算するSQL文}
    \label{code:sql-sum}
\end{figure}
\begin{table}[h]
    \centering
    \caption{「商品」テーブルから「メーカー」がB製菓のレコードに限定して「在庫数」の合計値を計算した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}r@{}}
    \toprule
...
コード\ref{code:sql-multiple-aggregates}は，「商品」テーブルから「メーカー」がB製菓のレコードだけに限定して，商品の「在庫数」の合計値，平均値，最大値と最小値の差を同時に計算するSQL文の例です（結果は表\ref{tab:sql-multiple-aggregates}）．
\begin{figure}[h]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{「商品」テーブルから「メーカー」がB製菓のレコードに限定して，「在庫数」の合計値，平均値，最大値と最小値の差を同時に計算するSQL文}
    \label{code:sql-multiple-aggregates}
\end{figure}
\begin{table}[h]
    \centering
    \caption{「商品」テーブルから「メーカー」がB製菓のレコードに限定して「在庫数」の合計値，平均値，最大値と最小値の差を同時に計算した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}rrr@{}}
    \toprule
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figure/sql-part1/groupby.pdf}
