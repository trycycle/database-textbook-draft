...
定義の話が続いてきたので，本章からは関係データベースを扱うための実践的な話題に移りましょう．
...
例えば，データ分析者は，分析に必要となるデータをデータベースから抽出する際にSQLを使っています．
また，ショッピングサイトでは，消費者がある商品の注文ボタンを押すと，ウェブアプリケーションのプログラムがSQLを通じてデータベースに注文記録を登録します．

ところで，SQLはデータベースに対する問い合わせ言語であって，プログラミング言語ではありません．
SQLはプログラミング言語に比べて覚えることは少なく，問い合わせ内容を英語に近い形で書き下すことができるよう設計されています．
もしあなたがデータ分析人材を目指しているのであれば，ソフトウェアを開発することはゴールではないでしょうから，高度なプログラミング能力を持つ必要はありません．
しかし，大量のデータを活かすことが期待されている今日，関係データベースに納められた大量のデータを自由自在に扱うには，\strong{SQLの習得は必須}です．
本章ではデータ分析の基本となる「読み出し（Read）」の基礎に絞って解説します．
より複雑な読み出しやレコードの登録，更新などは第\ref{chap:sql-part2}章で解説します．


\begin{notebox}{SQLの読み方}
SQLは「エス・キュー・エル」と読みます．
SQLは1970年代にIBM社が開発したデータベース管理システムSystem Rの操作言語SEQUELを起源としています\cite{SEQUEL}．
関係データベースに熱狂していた世代の人の中には，SEQUELの名残でSQLのことを「シークエル」と呼ぶ人もいます．
\end{notebox}

\begin{warningbox}{SQLにも方言がある}
RDBMSによって，SQLで使用できる関数が異なったりすることがあります．
特にSQLiteについては，簡易的なRDBMSということもあり，他のRDBMSでは実装されている機能や関数が実装されていない場合があります．
自分が書いたSQL文がうまく動作しない場合は，各RDBMSの公式ドキュメントなどを参照してみてください．
\end{warningbox}


\section{SQLと関係データモデルの比較}\label{sec:sql-vs-relational-data-model}
...
1つ目は，SQLでは同じ内容を示すレコードの存在（重複）が許されることがあるという点です．
関係データモデルにおいてSQLのレコードに相当するのがタプルです．
タプルは（数学的な）集合の要素であるため，関係データモデルでは同じ内容のタプルが重複して存在することは許されていません．
ところが，SQLでは問い合わせの結果の中では同じ内容のレコードの存在が許されています．
これは，現実の関係データベースに対する問い合わせにおいては，レコードの重複を許可しないと都合が悪い場合があるからです．
例えば，購買履歴を用いてあるカテゴリに属する商品の売り上げ高の平均額を計算することを考えましょう．
流れとしては，指定のカテゴリに属する各商品の売り上げ高を抽出し，その後平均値を計算するという運びになりますが，仮にカテゴリAに属する商品XとYの売り上げが同じ額の場合，（金額のみを）抽出した結果レコードには同じ内容のレコードが2つ含まれることになります．
このとき，もしSQLが同じ内容のレコードの重複を許さなければ，商品X，Yいずれかの売り上げ情報が無視され，売り上げ額の平均計算が正しく行えなくなります．

違いの2つ目は，SQLでは問い合わせ結果に対しては順序関係を定義できる点です．
関係データモデルでは関係中のタプルには順序関係がありません．
タプルは集合の要素であり，集合には順序の概念がないからです．
一方，SQLのテーブル内でもレコードに順序はない点は関係データモデルと共通ですが，問い合わせ結果については順序づけが可能です．
例えば，売り上げを記録している関係があったとき，売り上げ順に関係中のタプルを並べたくなる場合があります．
このようなニーズに応えるため，SQLでは問い合わせの結果得られたレコードに対しては順序づけを行うことができます．
...
用意した仮想データは，こちらのURL（\url{https://github.com/trycycle/database-textbook/raw/refs/heads/main/dataset/sql1-example.db}）よりダウンロードできます．
...
ダウンロードしたデータベースには，1章で取り上げたような小売店ショッピングサイトに掲載された商品に関する情報が格納されています．
具体的には，表\ref{tab:product-table}のように，「商品ID」「商品名」「カテゴリ」「メーカー」「価格」「在庫数」を列（属性）として持つテーブル「商品」が格納されています．
...
SQLによる代表的なクエリは，コード\ref{code:first-sql}に記したような形式の\strong{SELECT文}です．
\begin{figure}[!htbp]
...
このクエリは，
\begin{framed}
テーブル「商品」から列「在庫数」の値が100以上である行を見つけ，その行の列「商品名」を出力してください
\end{framed}
という問い合わせ要求を表現したものです．
これら要求を表現するために，コード\ref{code:first-sql}のクエリでは
\begin{itemize}
\item \graybox{SELECT}句を用いて「問い合わせ結果に表示したい列情報」，
\item \graybox{FROM}句を用いて「参照したいテーブル」，
...
\end{figure}
コード\ref{code:sql-structure}のようなテーブルからデータを抽出する基本的なSQL文では，\graybox{SELECT}句と\graybox{FROM}句を用います．
\graybox{SELECT}句は問い合わせ結果に表示したい情報を，\graybox{FROM}句には参照するテーブルをそれぞれ指定します．
...
例題として用いるテーブル「商品」には，列として「商品ID」「商品名」「カテゴリ」「メーカー」「価格」「在庫数」がありますが，用途によっては特定の列のデータのみ欲しい場合があります．
そのようなケースで用いるのが射影です．
例えば，テーブル「商品」から列「商品名」「メーカー」「価格」のデータのみを抽出するSQL文はコード\ref{code:sql-select}となります．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
テーブル「商品」には合計で500件のレコードが格納されているため，表が縦に長くなり，PCの画面からはみ出してしまいます．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」に対して射影を行い，その結果の先頭5件だけ表示するSQL文}
    \label{code:sql-select2}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」に対する射影の結果（先頭5件に限定）}
    \begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{商品名} & \textbf{メーカー} & \textbf{価格} \\ \midrule
    国産カレーパン & A製パン & 270 \\
...
真面目に記述するなら\graybox{SELECT}句にすべての列名を列挙することになりますが，テーブルが持つ列の数が多い場合は大変です．
...
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」に対する射影の結果（すべての列について，先頭の5件のレコードを表示）}
    \scalebox{0.93}{
    \begin{tabular}{@{}llllll@{}}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
...
\item $>=$（以上）
\end{itemize}
例えば，テーブル「商品」から列「価格」の値が500以上のレコードを抽出するSQL文はコード\ref{code:sql-where}のようになります．
このSQL文を実行すると，表\ref{tab:sql-where}のような結果が得られます．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から価格が500以上のレコードを抽出するSQL文}
    \label{code:sql-where}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」から価格が500以上のレコードを抽出した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}llllll@{}}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
...
コード\ref{code:sql-where2}は，テーブル「商品」から列「メーカー」の値が\graybox{Fファーム}のレコードを抽出するSQL文の例です．
このSQL文を実行すると，表\ref{tab:sql-where2}のような結果が得られます．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から列「メーカー」の値が\graybox{Fファーム}のレコードを抽出するSQL文}
    \label{code:sql-where2}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」から列「メーカー」の値が\graybox{Fファーム}のレコードを抽出した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}llllll@{}}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
...
そのようなケースでは\graybox{LIKE}演算子を用います．
以下の例のように，\graybox{WHERE}句内に\graybox{LIKE '\%部分文字列\%'}といった条件を書くと，指定した部分文字列を列データに含むレコードに絞り込むことができます．
なお，\%（パーセント）は0文字以上の任意の文字列を意味します．
以下は，テーブル「商品」から列「商品名」の値に\graybox{チョコ}の文字を含むレコードのみを抽出するSQL文の例です．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から列「商品名」の値に「チョコ」を含むレコードを抽出するSQL文}
    \label{code:sql-where3}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」から列「商品名」の値に「チョコ」を含むレコードを抽出した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}llllll@{}}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
...
もし\graybox{LIKE}演算子の条件を\graybox{\%チョコ\%}ではなく\graybox{\%チョコ}にした場合，例えば「無添加チョコチップパン」という商品名があった場合，末尾が「チョコ」でない商品名のレコードはマッチしなくなります．
...
コード\ref{code:sql-where4}は，テーブル「商品」から列「メーカー」の値が\graybox{N製菓}でかつ列「価格」の値が\graybox{500}以上のレコードを抽出するSQL文の例です．
このSQLを実行すると，表\ref{tab:sql-where4}のように，条件を満たすレコードが抽出され，その「商品名」「価格」「在庫数」の値が表示されます．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から列「メーカー」の値が\graybox{N製菓}でかつ列「価格」の値が\graybox{500}以上のレコードを抽出するSQL文（表示する列は「商品名」「価格」「在庫数」限定）}
    \label{code:sql-where4}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」から列「メーカー」の値が\graybox{N製菓}でかつ列「価格」の値が\graybox{500}以上のレコードを抽出し，列「商品名」「価格」「在庫数」の値を表示した結果}
    \scalebox{1.00}{
    \begin{tabular}{@{}lll@{}}
    \toprule
...
\section{重複の除外}
\ref{sec:sql-vs-relational-data-model}節で述べたように，SQL文の実行結果には重複するレコードが含まれることがあります．
例えば，テーブル「商品」の列「メーカー」に着目して，テーブル「商品」で取り扱っている商品のメーカーの一覧を抽出することを考えてみましょう．
これまで説明してきた内容を踏まえると，コード\ref{code:sql-undistinct}のように列「メーカー」を射影するSQL文でメーカーの一覧を抽出できると考えた方がいるかもしれません．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から列「メーカー」の値を抽出するSQL文}
    \label{code:sql-undistinct}
\end{figure}
ところが，期待に反してコード\ref{code:sql-undistinct}のSQL文を実行すると，表\ref{tab:sql-undistinct}のように列「メーカー」の値が重複しているレコードが抽出されてしまいます．
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」から列「メーカー」の値を抽出した結果}
    \scalebox{1.00}{
    \begin{tabular}{@{}l@{}}
    \toprule
    \textbf{メーカー} \\ \midrule
...
コード\ref{code:sql-undistinct}はコード\ref{code:sql-distinct}のように書き換えることで，重複を除いたレコード，つまりテーブル「商品」にある商品を取り扱っているメーカーの一覧を抽出することができます（結果は表\ref{tab:sql-distinct}）．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から列「メーカー」の値を重複を省いて抽出するSQL文}
    \label{code:sql-distinct}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」から列「メーカー」の値を重複を省いて抽出した結果}
    \scalebox{1.00}{
    \begin{tabular}{@{}l@{}}
    \toprule
    \textbf{メーカー} \\ \midrule
...
このSQL文を実行すると，表\ref{tab:sql-orderby}のように，テーブル「商品」から在庫数の少ない順にレコードが抽出されます．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から在庫数の少ない順にレコードを上位5件抽出するSQL文}
    \label{code:sql-orderby}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」から在庫数の少ない順にレコードを抽出した結果（上位5件）}
    \scalebox{0.93}{
    \begin{tabular}{@{}llllll@{}}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
...
コード\ref{code:sql-orderby2}は，テーブル「商品」から在庫数が多いもの順にレコードを抽出するSQL文の例です．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から在庫数の多い順にレコードを上位5件抽出するSQL文}
    \label{code:sql-orderby2}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」から在庫数の多い順にレコードを抽出した結果（上位5件）}
    \scalebox{0.93}{
    \begin{tabular}{@{}llllll@{}}
    \toprule
    \textbf{商品ID} & \textbf{商品名} & \textbf{カテゴリ} & \textbf{メーカー} & \textbf{価格} & \textbf{在庫数} \\ \midrule
...
（次節で解説する\graybox{GROUP BY}句を用いない場合）集約関数は，\strong{\graybox{WHERE}句で絞り込んだレコード全体を1つのグループと見なして}集計処理を行います．

例えば，テーブル「商品」から列「メーカー」の値が\graybox{L食品}のレコードだけに限定して，商品の「在庫数」の合計値を計算することを考えましょう．
合計値を求める集約関数は\graybox{SUM}です．
コード\ref{code:sql-sum}のように\graybox{SUM}関数の引数に「在庫数」を指定することで，「在庫数」の合計値を計算できます（結果は表\ref{tab:sql-sum}）．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から列「メーカー」の値が\graybox{L食品}のレコードに限定して「在庫数」の合計値を計算するSQL文}
    \label{code:sql-sum}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」から列「メーカー」の値が\graybox{L食品}のレコードに限定して「在庫数」の合計値を計算した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}r@{}}
    \toprule
...
コード\ref{code:sql-multiple-aggregates}は，テーブル「商品」から列「メーカー」の値が\graybox{L食品}のレコードだけに限定して，商品の「在庫数」の合計値，平均値，最大値と最小値の差を同時に計算するSQL文の例です（結果は表\ref{tab:sql-multiple-aggregates}）．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」から列「メーカー」の値が\graybox{L食品}のレコードに限定して，「在庫数」の合計値，平均値，最大値と最小値の差を同時に計算するSQL文}
    \label{code:sql-multiple-aggregates}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」から列「メーカー」の値が\graybox{L食品}のレコードに限定して「在庫数」の合計値，平均値，最大値と最小値の差を同時に計算した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}rrr@{}}
    \toprule
...
加算（+），減算（-），乗算（*），除算（/）といった四則演算のための演算子以外にも，余りを求めるための剰余演算子（\%），絶対値を求める\graybox{ABS}関数といった様々な算術演算ツールが用意されています．
気になった演算があればマニュアルを調べてみてください．
\end{notebox}

\graybox{COUNT}関数は行数（レコード数）をカウントすることができます．
\graybox{COUNT}関数の引数には，すべての列を意味するアスタリスク（*）が用いられることが多いです．
コード\ref{code:sql-count}は，テーブル「商品」に格納されたレコード数を調べるSQL文です（結果は表\ref{tab:sql-count}）．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」に格納されたレコード数を調べるSQL文}
    \label{code:sql-count}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」に格納されたレコード数を調べた結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}r@{}}
    \toprule
    \textbf{COUNT(*)} \\ \midrule
...
\graybox{COUNT}(列名) は，指定した列の値が「NULL（空値）ではないレコード」の件数を数え上げます．
要するに，\graybox{SELECT メーカー FROM 商品 WHERE メーカー IS NOT NULL;}\footnote{SQLではNULL値は特別な値なので，ある列の値がNULLであるかの判定には\graybox{IS NULL}や\graybox{IS NOT NULL}を使用します．\graybox{列名 = NULL}や\graybox{列名 != NULL}とは書きません．}の実行結果の行数を数えており，\graybox{SELECT COUNT(*) FROM 商品;}とは微妙に振る舞いが異なることに注意してください．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」に格納されたレコード数を調べるSQL文 - その2}
    \label{code:sql-count2}
\end{figure}
なお，コード\ref{code:sql-count2}の\graybox{COUNT(メーカー)}は，同じメーカー名が複数回登場した場合，それらも重複してカウントしてしまいます．
テーブル「商品」に現れるメーカーの種類数を数えたい場合は，コード\ref{code:sql-distinct-count}のSQL文のように重複を除いて数え上げたい列名の前に\graybox{DISTINCT}を付けましょう（結果は表\ref{tab:sql-distinct-count}）．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」に格納された商品のメーカー数を重複を除いて調べるSQL文}
    \label{code:sql-distinct-count}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」に格納された商品のメーカー数を重複を除いて調べた結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}r@{}}
    \toprule
...
例えば，コード\ref{code:sql-distinct-count}は，\graybox{SELECT COUNT(DISTINCT メーカー) AS メーカー数 FROM 商品;}とすることで，実行結果の列名を「メーカー数」に変更できます．
...
例えば，テーブル「商品」に対して問い合わせを行うSQL文内で「\graybox{GROUP BY} メーカー」と書くと，図\ref{fig:groupby}のようなイメージのグループ表が一時的に作成されます（点線がグループを表す）．
...
\begin{figure}[!htbp]
    ...
    \caption{GROUP BY句によってテーブル「商品」を列「メーカー」の値でグループ化したイメージ図}
    \label{fig:groupby}
\end{figure}

コード\ref{code:sql-groupby}は，テーブル「商品」を用いて，列「メーカー」の値ごとに商品の「在庫数」の平均値を計算するSQL文です．
このSQL文を実行すると，表\ref{tab:sql-groupby}のような結果が表示されます．
メーカー名を表示させるために，\graybox{SELECT}句に列「メーカー」を指定していることに注意してください．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」を用いて，列「メーカー」の値でグループ化し，メーカーごとに商品の「在庫数」の平均値を計算するSQL文}
    \label{code:sql-groupby}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」を用いて，列「メーカー」の値でグループ化し，メーカーごとに商品の「在庫数」の平均値を計算した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}ll@{}}
    \toprule
...
コード\ref{code:sql-groupby-where}は，テーブル「商品」の中で列「価格」の値が\graybox{360}円以上の商品が何件あるか，それら商品の平均価格はいくらかについて，メーカーごとに集計するSQL文です．
...
\begin{figure}[!htbp]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」を用いて，「メーカー」ごとに列「価格」の値が\graybox{360}円以上の商品の数と平均価格を集計するSQL文}
    \label{code:sql-groupby-where}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」を用いて，「メーカー」ごとに列「価格」の値が\graybox{360}円以上の商品の数と平均価格を集計した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}lll@{}}
    \toprule
...
上のケースでは集約「前」にレコードを絞り込んでいましたが，グループごとに集計した「後」に指定した条件を満たした結果を抽出したいケースもあります．
...
コード\ref{code:sql-groupby-having}は，テーブル「商品」のレコードについて，「メーカー」ごとに取り扱っている全商品の数と商品の平均価格を集約した後，商品の平均価格が\graybox{360}円以上のメーカーの情報だけを残して抽出するSQL文です．
実行結果は表\ref{tab:sql-groupby-having}のようになります．
\graybox{WHERE}句は集計前に行を絞り込み，\graybox{HAVING}句は集計後にグループを絞り込みます．
\graybox{WHERE}句を用いた場合と\graybox{HAVING}句を用いた場合で挙動が異なることを意識しましょう．
\begin{figure}[!htbp]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル「商品」を用いて，「メーカー」ごとに取り扱っている全商品の数と商品の平均価格を集約した後，平均価格が\graybox{360}円以上のメーカーの情報だけを抽出するSQL文}
    \label{code:sql-groupby-having}
\end{figure}
\begin{table}[!htbp]
    \centering
    \caption{テーブル「商品」を用いて，「メーカー」ごとに取り扱っている全商品の数と商品の平均価格を集約した後，平均価格が\graybox{360}円以上のメーカーの情報だけを抽出した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{メーカー} & \textbf{商品数} & \textbf{平均価格} \\ \midrule
...
本章では\graybox{SELECT}句，\graybox{FROM}句，\graybox{WHERE}句，\graybox{GROUP BY}句，\graybox{HAVING}句，\graybox{ORDER BY}句が登場しました．
...
まず冷蔵庫（\graybox{FROM}）から食材（テーブル）を出し，傷んだものを取り除き（\graybox{WHERE}），同じ種類の食材をまとめ（\graybox{GROUP BY}），まとめた分量が少なすぎるものについては省き（\graybox{HAVING}），お皿に盛り付けて（\graybox{SELECT}），最後に見栄え良く並べ替えます（\graybox{ORDER BY}）．
...
\graybox{SSDSE.db}には複数のテーブルが格納されていますが，このクイズで用いるのはテーブル``population''です．
表\ref{tab:population-table}の通り，テーブル``population''には，47ある各都道府県に関する総人口，小学校児童数，中学校生徒数，高等学校生徒数，大学学生数のデータが2021年度，2020年度分格納されています．
\begin{table}[!htbp]
    \centering
    \caption{SSDSE-Eからクイズ用に著者が抜粋・加工したデータ: テーブル``population''の内容}
    \scalebox{0.73}{
...
テーブル``population''を対象として，以下のSQL課題に取り組んでください．

\subsubsection{Q1. 射影}
列「都道府県」「調査年度」「大学学生数」に限定して，テーブル``population''のレコードを表示するSQL文を書きなさい．
なお，表示するレコード数は30件としなさい．

\subsubsection{Q2. 選択 (1/3)}
テーブル``population''内のレコードのうち，列「都道府県」の値が\graybox{愛知県}で列「調査年度」の値が\graybox{2020}であるものを表示するSQL文を書きなさい．

\subsubsection{Q3. 選択 (2/3)}
テーブル``population''内のレコードのうち
\begin{itemize}
\item 列「総人口」の値が\graybox{300万人}未満，かつ
\item 列「都道府県」の値が\graybox{県}で終わる，かつ
\item 列「中学校生徒数」の値が列「高等学校生徒数」の値以下
\end{itemize}
...
テーブル``population''内のレコードのうち，列「調査年度」の値が\graybox{2021}のものについて，列「大学学生数」の値が大きいもの順に並び替えて表示するSQL文を書きなさい．

\subsubsection{Q6. ソート (2/2)}
テーブル``population''内のレコードのうち，列「調査年度」の値が\graybox{2021}のものについて，列「総人口」に占める列「大学学生数」の割合が大きいもの順に並び替えて表示するSQL文を書きなさい．
その際，列「総人口」に占める列「大学学生数」の割合も合わせて表示しなさい．
なお，割合の計算では1.0を掛けるなどして実数として計算すること．

\subsubsection{Q7. GROUP BY (1/3)}
テーブル``population''内のレコードに対して都道府県ごとの集約演算を行い，都道府県別に
\begin{itemize}
...
テーブル``population''内の総人口が\graybox{300万人}を超えるレコードに対して調査年度ごとの集約演算を行い，
調査年度別に
\begin{itemize}
...
テーブル``population''のレコードに対して都道府県ごとの集約演算を行い，
都道府県別に
\begin{itemize}
\item 「（調査期間中の）総人口」の平均
...
\end{itemize}
を求めるSQL文を書きなさい．
ただし，「（調査期間中の）高等学校生徒数と大学学生数の差」の平均が\graybox{10000}以下になるものだけを表示しなさい．
