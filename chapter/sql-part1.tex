...
より複雑な読み出しやレコードの登録，更新などは\ref{chap:sql-part2}で解説します．
...
\begin{figure}[tb]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル\graybox{商品}に対して射影を行い，その結果の先頭5件だけ表示するSQL文}
    \label{code:sql-select2}
\end{figure}
\begin{table}[tb]
    \centering
    \centering
    \caption{テーブル\graybox{商品}に対する射影の結果（先頭5件に限定）}
    \begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{商品名} & \textbf{メーカー} & \textbf{価格} \\ \midrule
    国産カレーパン & A製パン & 270 \\
...
もし\graybox{LIKE}演算子の条件を\graybox{\%チョコ\%}ではなく\graybox{\%チョコ}にした場合，「無添加 チョコチップパン」のように文末がチョコで終わらない商品名のレコードはマッチしなくなります．
...
このSQLを実行すると，表\ref{tab:sql-where4}のように，列「メーカー」の値が\graybox{N製菓}
でかつ列「価格」の値が\graybox{500}以上のレコードが抽出され，その「商品名」「価格」「在庫数」の値が表示されます．
\begin{figure}[h]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
コード\ref{code:sql-undistinct}はコード\ref{code:sql-distinct}のように書き換えることで，重複を除いたレコード，つまりテーブル「商品」にある商品を取り扱っているメーカーの一覧を抽出することができます（結果は\ref{tab:sql-distinct}）．
\begin{figure}[h]
    \begin{minted}[gobble=1,
        bgcolor=background,
        fontsize=\small,
...
\graybox{SUM}や\graybox{AVG}といった集約関数は，\graybox{NULL}値を無視して計算を行います．
例えば，\graybox{AVG}関数を使って平均値を計算する際，分母の件数には\graybox{NULL}値を含むレコードはカウントされないので注意しましょう．
\end{notebox}
...
\begin{figure}[h]
    ...
    \end{minted}
    \captionsetup{name=コード}
    \caption{テーブル\graybox{商品}を用いて，「メーカー」ごとに取り扱っている全商品の数と商品の平均価格を集約した後，平均価格が\graybox{360}円以上のメーカーの情報だけを抽出するSQL文}
    \label{code:sql-groupby-having}
\end{figure}
\begin{table}[h]
    \centering
    \caption{テーブル\graybox{商品}を用いて，「メーカー」ごとに取り扱っている全商品の数と商品の平均価格を集約した後，平均価格が\graybox{360}円以上のメーカーの情報だけを抽出した結果}
    \scalebox{0.93}{
    \begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{メーカー} & \textbf{商品数} & \textbf{平均価格} \\ \midrule
